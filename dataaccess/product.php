...
function doList(){
    ...
    $SQL = "SELECT p.id id ,p.name name ,p.image image,p.intro intro,pc.name cname 
            FROM $entryName p inner join catalog pc on pc.id=p.catalogId  ORDER BY $sidx $sord LIMIT $start , $limit";
    ...
    while($row = mysql_fetch_assoc($result)) {
        ...
        $responce->rows[$i]['image']=$row['image'];
