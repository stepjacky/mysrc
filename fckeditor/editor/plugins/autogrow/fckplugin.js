	...
	Check : function() {
		...
		if (delta != 0) {
			...
			if (newHeight != window.frameElement.height) {
				...
				// Gecko browsers use an onresize handler to update the
				// innermost
				// IFRAME's height. If the document is modified before the
				// onresize
				// is triggered, the plugin will miscalculate the new height.
				// Thus,
	...
	GetHeightDelta : function() {
		...
		if (FCKBrowserInfo.IsIE) {
			...
			iInnerHeight = oInnerDoc.body.offsetHeight
					+ (parseInt(FCKDomTools.GetCurrentElementStyle(
									oInnerDoc.body, 'margin-top'), 10) || 0)
					+ (parseInt(FCKDomTools.GetCurrentElementStyle(
									oInnerDoc.body, 'margin-bottom'), 10) || 0);
		}
