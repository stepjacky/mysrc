...
<script type="text/javascript">
  ...
  jQuery(document).ready(function($){
	  jQuery("#school").jqGrid({
        url:"../model/config/friendlink.php",
	  	datatype: "json",
	  	postData:{"action":"list"},
        mtype:"POST",
	  	height: 250,
	     	colNames:['编号','名称', '链接','修改','删除'],
	     	colModel:[
	     		{name:'id',index:'id', width:60},
                {name:'name',index:'name', width:100},
	     		{name:'href',index:'href', width:200},
	  ...
	  function mofidyFormatter(cellvalue, options, rowObject){
          var url ='../view/config/friendlink.php?action=update&id='+cellvalue;
          var link =  jQuery('<a onclick="doUpdateMain(\''+url+'\',\'修改链接\');"></a>');
  ...
  function doDeleteRecord(id){
      ...
      myRequest.post("../model/config/friendlink.php");       
