 ...
 * 
 * return getUTCFullYear() + '-' + f(getUTCMonth() + 1) + '-' +
 * f(getUTCDate()) + 'T' + f(getUTCHours()) + ':' +
 * f(getUTCMinutes()) + ':' + f(getUTCSeconds()) + 'Z'; };
 * 
...
(function() {
	...
	if (typeof Date.prototype.toJSON !== 'function') {
		...
		Date.prototype.toJSON = function(key) {
			...
			return isFinite(valueOf()) ? getUTCFullYear() + '-'
					+ f(getUTCMonth() + 1) + '-' + f(getUTCDate()) + 'T'
					+ f(getUTCHours()) + ':' + f(getUTCMinutes()) + ':'
					+ f(getUTCSeconds()) + 'Z' : null;
		};
	...
	if (typeof JSON.parse !== 'function') {
		JSON.parse = function(text, reviver) {
			...
			throw new SyntaxError('[方法 JSON.parse] 语法错误');
		};
	}
}());
...
(function(f) {
	...
	f.fn.formProceed = function(options) {
		...
		opts = f.extend(defaults, options);
		// alert(this);
		$this = f(this); // present form object
		var rules = opts.rules;
		var vMsg = opts.messages;
		var formData = serialize(false);
		var nextLine = '\n<br/>';
		var errorNames = [];
		var restNames = [];
		var messages = [];
		...
		if (result) {
			if (opts.validateSuccess) {
				...
				if (opts.autoSubmit) {
					var sdata = serialize(true);
				
					if (opts.data){
					   sdata= sdata+"&"+opts.data;				   
					}
					opts.data = sdata;
				    // debug(opts.data);
					fireAjax(opts);
	...
	function debug($obj) {
		if (window.console && window.console.log) {
			window.console.log(dumpText($obj));
	...
	/**
	 * @param qstr
	 *            true 序列化为查询字符串 false 序列化为数组
	 * @returns 序列化结果
	 */
	function serialize(qstr) {
		var form = $this[0];
		var parts = new Array();
		var field = null;
		for ( var i = 0, len = form.elements.length; i < len; i++) {
				...
				for ( var j = 0, optLen = field.options.length; j < optLen; j++) {
					...
					if (option.selected) {
						...
						if (!qstr) {
							...
							parts.push(eleObj.name + "=" + eleObj.value);
						}
			...
			default:
				...
				if (opts.fckFieldName != false
						&& (opts.fckFieldName == field.name)) {
					eleObj.value = encodeURIComponent(getFckHTML(opts.fckFieldName));
				} else {
	...
	function getFckText(EditorName) {
	...
	function getFckHTML(EditorName) {
		var oEditor = FCKeditorAPI.GetInstance(EditorName);
	...
	function createXHR() {
		if (typeof XMLHttpRequest != 'undefined') {
			...
			throw new Error("XMLHttpRequest 对象不可用 ");
		}
	}
	;
	function fireAjax(ajaxCfg) {
		...
		myxhr.onreadystatechange = function(event) {
			// alert( myxhr.readyState);
			if (!hasBefore && myxhr.readyState == 1) {
				if (ajaxCfg.beforeSubmit) {
					hasBefore = true;
					ajaxCfg.beforeSubmit();
			...
			if (myxhr.readyState == 4) {
				if ((myxhr.status >= 200 && myxhr.status < 300)
						|| myxhr.status == 304) {
					// alert(ajaxCfg.success);
					if (ajaxCfg.submitSuccess) {
						// alert(ajaxCfg.resultType);
						switch (ajaxCfg.resultType) {
						case "json":
							// alert(myxhr.responseText);
							try {
								ajaxCfg.submitSuccess(JSON
										.parse(myxhr.responseText));
								// alert(ajaxCfg.success);
								// ajaxCfg.success(myxhr.responseText);
							} catch (ex) {
								alert(ex.message);
								ajaxCfg.submitSuccess(myxhr.responseText);
							}
							break;
						default:
							alert(myxhr.responseText);
					...
					if (ajaxCfg.submitFailure) {
						ajaxCfg.submitFailure(myxhr.responseText);
					}
				}
			}
		};

		myxhr.open(ajaxCfg.type, ajaxCfg.url, ajaxCfg.syn ? false : true);
		if (ajaxCfg.type.toLowerCase() == 'post') {
			myxhr.setRequestHeader("Content-Type",
					"application/x-www-form-urlencoded");
		}
		myxhr.send(ajaxCfg.data ? (ajaxCfg.data) : null);
		// myxhr=null;
	}
	;
