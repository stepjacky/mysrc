...
t;var A=f.rowNum,B=g.scrollTop=g.bDiv.scrollTop,M=Math.round(s.position().top)-B,I=M+s.height();t=t*A;var P,J,H;if(M<=0&&I<o&&(f.lastpage==null||parseInt((I+B+t-1)/t,10)<f.lastpage)){J=parseInt((o-I+t-1)/t,10);if(I>=0||J<2||f.scroll===true){P=Math.round((I+B)/t)+1;M=-1}else M=1}if(M>0){P=parseInt(B/t,10)+1;J=parseInt((B+o)/t,10)+2-P;H=true}if(J)if(!(f.lastpage&&P>f.lastpage))if(g.hDiv.loading)g.timer=setTimeout(g.populateVisible,200);else{f.page=P;if(H){g.selectionPreserver(s[0]);g.emptyRows(g.bDiv)}g.populate(J)}}}},
scrollGrid:function(){if(f.scroll){var o=g.bDiv.scrollTop;if(o!=g.scrollTop){g.scrollTop=o;g.timer&&clearTimeout(g.timer);g.timer=setTimeout(g.populateVisible,200)}}g.hDiv.scrollLeft=g.bDiv.scrollLeft;if(f.footerrow)g.sDiv.scrollLeft=g.bDiv.scrollLeft},selectionPreserver:function(o){var s=o.p,t=s.selrow,A=s.selarrrow?a.makeArray(s.selarrrow):null,B=o.grid.bDiv.scrollLeft,M=s.gridComplete;s.gridComplete=function(){s.selrow=null;s.selarrrow=[];if(s.multiselect&&A&&A.length>0)for(var I=0;I<A.length;I++)A[I]!=
t&&a(o).jqGrid("setSelection",A[I],false);t&&a(o).jqGrid("setSelection",t,false);o.grid.bDiv.scrollLeft=B;if(s.gridComplete=M)M()}}};this.p=f;var h,j,c;if(this.p.colNames.length===0)for(h=0;h<this.p.colModel.length;h++)this.p.colNames[h]=this.p.colModel[h].label||this.p.colModel[h].name;if(this.p.colNames.length!==this.p.colModel.length)alert(a.jgrid.errors.model);else{var k=a("<div class='ui-jqgrid-view'></div>"),l,p=a.browser.msie?true:false,m=a.browser.safari?true:false;c=this;c.p.direction=a.trim(c.p.direction.toLowerCase());
if(a.inArray(c.p.direction,["ltr","rtl"])==-1)c.p.direction="ltr";j=c.p.direction;a(k).insertBefore(this);a(this).appendTo(k).removeClass("scroll");var n=a("<div class='ui-jqgrid ui-widget ui-widget-content ui-corner-all'></div>");a(n).insertBefore(k).attr({id:"gbox_"+this.id,dir:j});a(k).appendTo(n).attr("id","gview_"+this.id);l=p&&a.browser.version<=6?'<iframe style="display:block;position:absolute;z-index:-1;filter:Alpha(Opacity=\'0\');" src="javascript:false;"></iframe>':"";a("<div class='ui-widget-overlay jqgrid-overlay' id='lui_"+
this.id+"'></div>").append(l).insertBefore(k);a("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(k);a(this).attr({cellSpacing:"0",cellPadding:"0",border:"0",role:"grid","aria-multiselectable":!!this.p.multiselect,"aria-labelledby":"gbox_"+this.id});var r=function(o,s){o=parseInt(o,10);return isNaN(o)?s?s:0:o},q=function(o,s){var t=c.p.colModel[o].align,A='style="',B=c.p.colModel[o].classes;if(t)A+="text-align:"+t+";";if(c.p.colModel[o].hidden===
true)A+="display:none;";if(s===0)A+="width: "+g.headers[o].width+"px;";return A+'"'+(B!==undefined?' class="'+B+'"':"")},u=function(o,s,t,A,B){o=v(o,s,t,B,"add");return'<td role="gridcell" '+q(t,A)+' title="'+a.jgrid.stripHtml(o)+'">'+o+"</td>"},v=function(o,s,t,A,B){t=c.p.colModel[t];if(typeof t.formatter!=="undefined"){o={rowId:o,colModel:t,gid:c.p.id};s=a.isFunction(t.formatter)?t.formatter.call(c,s,o,A,B):a.fmatter?a.fn.fmatter(t.formatter,s,o,A,B):w(s)}else s=w(s);return s},w=function(o){return o===
undefined||o===null||o===""?"&#160;":c.p.autoencode?a.jgrid.htmlEncode(o):o+""},z=function(o,s,t){o='<input type="checkbox" id="jqg_'+o+'" class="cbox" name="jqg_'+o+'"/>';return"<td role='gridcell' "+q(s,t)+">"+o+"</td>"},D=function(o,s,t,A){t=(parseInt(t,10)-1)*parseInt(A,10)+1+s;return'<td role="gridcell" class="ui-state-default jqgrid-rownum" '+q(o,s)+">"+t+"</td>"},C=function(o){var s,t=[],A=0,B;for(B=0;B<c.p.colModel.length;B++){s=c.p.colModel[B];if(s.name!=="cb"&&s.name!=="subgrid"&&s.name!==
"rn"){t[A]=o=="xml"?s.xmlmap||s.name:s.jsonmap||s.name;A++}}return t},y=function(o){var s=c.p.remapColumns;if(!s||!s.length)s=a.map(c.p.colModel,function(t,A){return A});if(o)s=a.map(s,function(t){return t<o?null:t-o});return s},G=function(o,s){var t=a("tbody:first",o);if(!c.p.gridview||c.p.jqgdnd)a("*",t).children().unbind();if(p)a.jgrid.empty.apply(t[0]);else t[0].innerHTML="";if(s&&c.p.scroll){a(">div:first",o).css({height:"auto"}).children("div:first").css({height:0,display:"none"});o.scrollTop=
0}},x=function(o,s){var t,A,B;t=o[s];if(t===undefined){if(typeof s==="string")B=s.split(".");try{if(B.length)for(t=o;t&&B.length;){A=B.shift();t=t[A]}}catch(M){}}return t},N=function(o,s,t,A,B){var M=new Date;c.p.reccount=0;if(a.isXMLDoc(o)){if(c.p.treeANode===-1&&!c.p.scroll){G(s);t=0}else t=t>0?t:0;var I,P=0,J,H,O=0,X=0,ca=0,ha,pa,na=[],ra,ja={},Da=c.rows.length,ia,Aa,fa=[],ua=0,Ea=c.p.altRows===true?" "+c.p.altclass:"";c.p.xmlReader.repeatitems||(na=C("xml"));ha=c.p.keyIndex===false?c.p.xmlReader.id:
...
this.text;if(c.p.lastpage===undefined)c.p.lastpage=1});a(c.p.xmlReader.records,o).each(function(){c.p.records=this.textContent||this.text||0});a(c.p.xmlReader.userdata,o).each(function(){c.p.userData[this.getAttribute("name")]=this.textContent||this.text});o=a(c.p.xmlReader.root+" "+c.p.xmlReader.row,o);var Fa=o.length,qa=0;if(o&&Fa){var Ha=parseInt(c.p.rowNum,10),Ja=c.p.scroll?(parseInt(c.p.page,10)-1)*Ha+1:1;if(B)Ha*=B+1;for(B=a.isFunction(c.p.afterInsertRow);qa<Fa;){ia=o[qa];Aa=pa(ia,Ja+qa);I=
t==0?0:t+1;I=(I+qa)%2==1?Ea:"";fa[ua++]='<tr id="'+Aa+'" role="row" class ="ui-widget-content jqgrow ui-row-'+c.p.direction+""+I+'">';if(c.p.rownumbers===true){fa[ua++]=D(0,qa,c.p.page,c.p.rowNum);ca=1}if(c.p.multiselect===true){fa[ua++]=z(Aa,ca,qa);O=1}if(c.p.subGrid===true){fa[ua++]=a(c).jqGrid("addSubGridCell",O+ca,qa+t);X=1}if(c.p.xmlReader.repeatitems){ra||(ra=y(O+X+ca));var Ka=a(c.p.xmlReader.cell,ia);a.each(ra,function(Ba){var ya=Ka[this];if(!ya)return false;J=ya.textContent||ya.text;ja[c.p.colModel[Ba+
...
1],O+ca)}catch(Ma){}B&&c.p.afterInsertRow.call(c,Aa,ja,ia);fa=[];ua=0}ja={};P++;qa++;if(P==Ha)break}}c.p.gridview===true&&a("tbody:first",s).append(fa.join(""));c.p.totaltime=new Date-M;if(P>0){c.grid.cols=c.rows[0].cells;if(c.p.records===0)c.p.records=Fa}fa=null;if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=P;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);A||da(false,true)}},Y=function(o,s,t,A,B){var M=new Date;c.p.reccount=0;if(o){if(c.p.treeANode===
-1&&!c.p.scroll){G(s);t=0}else t=t>0?t:0;var I=0,P,J,H,O=[],X,ca=0,ha=0,pa=0,na,ra,ja,Da={},ia,Aa=c.rows.length,fa;H=[];ia=0;var ua=c.p.altRows===true?" "+c.p.altclass:"";c.p.page=x(o,c.p.jsonReader.page)||1;ja=x(o,c.p.jsonReader.total);c.p.lastpage=ja===undefined?1:ja;c.p.records=x(o,c.p.jsonReader.records)||0;c.p.userData=x(o,c.p.jsonReader.userdata)||{};c.p.jsonReader.repeatitems||(X=O=C("json"));ja=c.p.keyIndex===false?c.p.jsonReader.id:c.p.keyIndex;if(O.length>0&&!isNaN(ja)){if(c.p.remapColumns&&
c.p.remapColumns.length)ja=a.inArray(ja,c.p.remapColumns);ja=O[ja]}if(ra=x(o,c.p.jsonReader.root)){na=ra.length;o=0;var Ea=parseInt(c.p.rowNum,10),Fa=c.p.scroll?(parseInt(c.p.page,10)-1)*Ea+1:1;if(B)Ea*=B+1;for(var qa=a.isFunction(c.p.afterInsertRow);o<na;){B=ra[o];fa=B[ja];if(fa===undefined){fa=Fa+o;if(O.length===0)if(c.p.jsonReader.cell)fa=B[c.p.jsonReader.cell][ja]||fa}P=t==0?0:t+1;P=(P+o)%2==1?ua:"";H[ia++]='<tr id="'+fa+'" role="row" class= "ui-widget-content jqgrow ui-row-'+c.p.direction+""+
P+'">';if(c.p.rownumbers===true){H[ia++]=D(0,o,c.p.page,c.p.rowNum);pa=1}if(c.p.multiselect){H[ia++]=z(fa,pa,o);ca=1}if(c.p.subGrid){H[ia++]=a(c).jqGrid("addSubGridCell",ca+pa,o+t);ha=1}if(c.p.jsonReader.repeatitems){if(c.p.jsonReader.cell)B=x(B,c.p.jsonReader.cell);X||(X=y(ca+ha+pa))}for(J=0;J<X.length;J++){P=x(B,X[J]);H[ia++]=u(fa,P,J+ca+ha+pa,o+t,B);Da[c.p.colModel[J+ca+ha+pa].name]=P}H[ia++]="</tr>";if(c.p.gridview===false){if(c.p.treeGrid===true){ia=c.p.treeANode>=-1?c.p.treeANode:0;H=a(H.join(""))[0];
try{a(c).jqGrid("setTreeNode",Da,H)}catch(Ha){}Aa===0?a("tbody:first",s).append(H):a(c.rows[o+ia+t]).after(H)}else a("tbody:first",s).append(H.join(""));if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",c.rows[c.rows.length-1],ca+pa)}catch(Ja){}qa&&c.p.afterInsertRow.call(c,fa,Da,B);H=[];ia=0}Da={};I++;o++;if(I==Ea)break}c.p.gridview===true&&a("tbody:first",s).append(H.join(""));c.p.totaltime=new Date-M;if(I>0){c.grid.cols=c.rows[0].cells;if(c.p.records===0)c.p.records=na}}if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=
0;c.p.reccount=I;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);A||da(false,true)}},da=function(o,s){var t,A,B,M,I,P,J,H="";B=(parseInt(c.p.page,10)-1)*parseInt(c.p.rowNum,10);I=B+c.p.reccount;if(c.p.scroll){t=a("tbody:first > tr",c.grid.bDiv);B=I-t.length;if(A=t.outerHeight()){t=B*A;A=parseInt(c.p.records,10)*A;a(">div:first",c.grid.bDiv).css({height:A}).children("div:first").css({height:t,display:t?"":"none"})}}H=c.p.pager?c.p.pager:"";H+=c.p.toppager?H?
","+c.p.toppager:c.p.toppager:"";if(H){J=a.jgrid.formatter.integer||{};if(c.p.loadonce){t=A=1;c.p.lastpage=c.page=1;a(".selbox",H).attr("disabled",true)}else{t=r(c.p.page,1);A=r(c.p.lastpage,1);a(".selbox",H).attr("disabled",false)}if(c.p.pginput===true){a(".ui-pg-input",H).val(c.p.page);a("#sp_1",H).html(a.fmatter?a.fmatter.util.NumberFormat(c.p.lastpage,J):c.p.lastpage)}if(c.p.viewrecords)if(c.p.reccount===0)a(".ui-paging-info",H).html(c.p.emptyrecords);else{M=B+1;P=c.p.records;if(a.fmatter){M=
...
c.p.toppager).removeClass("ui-state-disabled")}if(t==A){a("#next, #last",c.p.pager).addClass("ui-state-disabled").removeClass("ui-state-hover");c.p.toppager&&a("#next_t, #last_t",c.p.toppager).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#next, #last",c.p.pager).removeClass("ui-state-disabled");c.p.toppager&&a("#next_t, #last_t",c.p.toppager).removeClass("ui-state-disabled")}}}o===true&&c.p.rownumbers===true&&a("td.jqgrid-rownum",c.rows).each(function(O){a(this).html(B+1+O)});
s&&c.p.jqgdnd&&a(c).jqGrid("gridDnD","updateDnD");a.isFunction(c.p.gridComplete)&&c.p.gridComplete.call(c)},Q=function(o){if(!c.grid.hDiv.loading){var s=c.p.scroll&&o==false,t={},A,B=c.p.prmNames;if(B.search!==null)t[B.search]=c.p.search;if(B.nd!=null)t[B.nd]=(new Date).getTime();if(B.rows!==null)t[B.rows]=c.p.rowNum;if(B.page!==null)t[B.page]=c.p.page;if(B.sort!==null)t[B.sort]=c.p.sortname;if(B.order!==null)t[B.order]=c.p.sortorder;var M=c.p.loadComplete,I=a.isFunction(M);I||(M=null);var P=0;o=
o||1;if(o>1)if(B.npage!=null){t[B.npage]=o;P=o-1;o=1}else M=function(H){I&&c.p.loadComplete.call(c,H);c.grid.hDiv.loading=false;c.p.page++;Q(o-1)};else B.npage!=null&&delete c.p.postData[B.npage];a.extend(c.p.postData,t);var J=!c.p.scroll?0:c.rows.length-1;if(a.isFunction(c.p.datatype))c.p.datatype.call(c,c.p.postData,"load_"+c.p.id);else{a.isFunction(c.p.beforeRequest)&&c.p.beforeRequest.call(c);A=c.p.datatype.toLowerCase();switch(A){case "json":case "jsonp":case "xml":case "script":a.ajax(a.extend({url:c.p.url,
...
s=[],t=0,A,B,M,I=c.p.sortorder=="asc"?1:-1;a.each(c.p.colModel,function(J){if(this.index==c.p.sortname||this.name==c.p.sortname){t=c.p.lastsort=J;A=this.sorttype;return false}});M=A=="float"||A=="number"||A=="currency"?function(J){J=parseFloat(J.replace(o,""));return isNaN(J)?0:J}:A=="int"||A=="integer"?function(J){return r(J.replace(o,""),0)}:A=="date"||A=="datetime"?function(J){return sa(c.p.colModel[t].datefmt||"Y-m-d",J).getTime()}:function(J){return a.trim(J.toUpperCase())};a.each(c.rows,function(J,
H){try{B=a.unformat(a(H).children("td").eq(t),{rowId:H.id,colModel:c.p.colModel[t]},t,true)}catch(O){B=a(H).children("td").eq(t).text()}H.sortKey=M(B);s[J]=this});if(c.p.treeGrid)a(c).jqGrid("SortTree",I);else{s.sort(function(J,H){if(J.sortKey<H.sortKey)return-I;if(J.sortKey>H.sortKey)return I;return 0});if(s[0]){a("td",s[0]).each(function(J){a(this).css("width",g.headers[J].width+"px")});c.grid.cols=s[0].cells}var P="";if(c.p.altRows)P=c.p.altclass;a.each(s,function(J,H){if(P)J%2==1?a(H).addClass(P):
a(H).removeClass(P);a("tbody",c.grid.bDiv).append(H);H.sortKey=null})}c.grid.bDiv.scrollTop=0},sa=function(o,s){var t={m:1,d:1,y:1970,h:0,i:0,s:0},A,B,M;s=s.split(/[\\\/:_;.\t\T\s-]/);o=o.split(/[\\\/:_;.\t\T\s-]/);var I=a.jgrid.formatter.date.monthNames;A=0;for(B=o.length;A<B;A++){if(o[A]=="M"){M=a.inArray(s[A],I);if(M!==-1&&M<12)s[A]=M+1}if(o[A]=="F"){M=a.inArray(s[A],I);if(M!==-1&&M>11)s[A]=M+1-12}t[o[A].toLowerCase()]=parseInt(s[A],10)}t.m=parseInt(t.m,10)-1;o=t.y;if(o>=70&&o<=99)t.y=1900+t.y;
else if(o>=0&&o<=69)t.y=2E3+t.y;return new Date(t.y,t.m,t.d,t.h,t.i,t.s,0)};l=function(o,s){var t="",A="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",B="",M,I,P,J,H=function(O){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];if(a.isFunction(c.p.onPaging))if(c.p.onPaging.call(c,O)=="stop")return false;return true};o=o.substr(1);M="pg_"+o;I=o+"_left";
P=o+"_center";J=o+"_right";a("#"+o).append("<div id='"+M+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' role='row'><tbody><tr><td id='"+I+"' align='left'></td><td id='"+P+"' align='center' style='white-space:pre;'></td><td id='"+J+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(c.p.rowList.length>0){B="<td dir='"+j+"'>";B+="<select class='ui-pg-selbox' role='listbox'>";for(I=
0;I<c.p.rowList.length;I++)B+="<option role='option' value='"+c.p.rowList[I]+"'"+(c.p.rowNum==c.p.rowList[I]?" selected":"")+">"+c.p.rowList[I]+"</option>";B+="</select></td>"}if(j=="rtl")A+=B;if(c.p.pginput===true)t="<td dir='"+j+"'>"+a.jgrid.format(c.p.pgtext||"","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1'></span>")+"</td>";if(c.p.pgbuttons===true){I=["first"+s,"prev"+s,"next"+s,"last"+s];j=="rtl"&&I.reverse();A+="<td id='"+I[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-first'></span></td>";
...
"")A+=t;if(j=="ltr")A+=B;A+="</tr></tbody></table>";c.p.viewrecords===true&&a("td#"+o+"_"+c.p.recordpos,"#"+M).append("<div dir='"+j+"' style='text-align:"+c.p.recordpos+"' class='ui-paging-info'></div>");a("td#"+o+"_"+c.p.pagerpos,"#"+M).append(A);B=a(".ui-jqgrid").css("font-size")||"11px";a("body").append("<div id='testpg' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+B+";visibility:hidden;' ></div>");A=a(A).clone().appendTo("#testpg").width();a("#testpg").remove();if(A>0){if(t!=
...
else{c.p.selrow=null;c.p.multiselect&&a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false);c.p.selarrrow=[];c.p.savedRow=[];c.p.scroll&&G(c.grid.bDiv,true)}c.p.subGrid&&c.p.datatype=="local"&&a("td.sgexpanded","#"+c.p.id).each(function(){a(this).trigger("click")});Q();if(c.p.sortname!=o&&s)c.p.lastsort=s}},Ca=function(o){var s=o,t;for(t=o+1;t<c.p.colModel.length;t++)if(c.p.colModel[t].hidden!==true){s=t;break}return s-o},va=function(o){var s,t={},A=m?0:c.p.cellLayout;for(s=t[0]=t[1]=t[2]=
...
records:"rows>records",repeatitems:true,cell:"cell",id:"[id]",userdata:"userdata",subgrid:{root:"rows",row:"row",repeatitems:true,cell:"cell"}},c.p.xmlReader);c.p.jsonReader=a.extend(true,{root:"rows",page:"page",total:"total",records:"records",repeatitems:true,cell:"cell",id:"id",userdata:"userdata",subgrid:{root:"rows",repeatitems:true,cell:"cell"}},c.p.jsonReader);if(c.p.scroll){c.p.pgbuttons=false;c.p.pginput=false;c.p.rowList=[]}var F="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",V,
...
L+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+j+"'></span></span>";for(h=0;h<this.p.colNames.length;h++){F+="<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+j+"'>";L=c.p.colModel[h].index||c.p.colModel[h].name;F+="<div id='jqgh_"+c.p.colModel[h].name+"' "+V+">"+c.p.colNames[h];if(L==c.p.sortname)c.p.lastsort=h;F+=ba+"</div></th>"}F+="</tr></thead>";a(this).append(F);a("thead tr:first th",this).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});
...
"false");ka[o]=this.id}});c.p.selarrrow=[];c.p.selrow=null;aa=false}if(Z)c.p.onSelectAll.call(c,aa?c.p.selarrrow:ka,aa)})}a.each(c.p.colModel,function(){if(!this.width)this.width=150;this.width=parseInt(this.width,10)});if(c.p.autowidth===true){F=a(n).innerWidth();c.p.width=F>0?F:"nw"}(function(){var o=0,s=c.p.cellLayout,t=0,A,B=c.p.scrollOffset,M,I=false,P,J=0,H=0,O=0,X;if(m)s=0;a.each(c.p.colModel,function(){if(typeof this.hidden==="undefined")this.hidden=false;if(this.hidden===false){o+=r(this.width,
0);if(this.fixed){J+=this.width;H+=this.width+s}else t++;O++}});if(isNaN(c.p.width))c.p.width=g.width=o;else g.width=c.p.width;c.p.tblwidth=o;if(c.p.shrinkToFit===false&&c.p.forceFit===true)c.p.forceFit=false;if(c.p.shrinkToFit===true&&t>0){P=g.width-s*t-H;if(!isNaN(c.p.height)){P-=B;I=true}o=0;a.each(c.p.colModel,function(ca){if(this.hidden===false&&!this.fixed){this.width=M=Math.floor(P/(c.p.tblwidth-J)*this.width);o+=M;A=ca}});X=0;if(I){if(g.width-H-(o+s*t)!==B)X=g.width-H-(o+s*t)-B}else if(!I&&
Math.abs(g.width-H-(o+s*t))!==1)X=g.width-H-(o+s*t);c.p.colModel[A].width+=X;c.p.tblwidth=o+X+J+O*s}})();a(n).css("width",g.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+c.p.id+"'>&#160;</div>");a(k).css("width",g.width+"px");F=a("thead:first",c).get(0);var wa="<table role='grid' style='width:"+c.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+j+"'>";k=a("tr:first",F);c.p.disableClick=
false;a("th",k).each(function(o){a("div",this);W=c.p.colModel[o].width;if(typeof c.p.colModel[o].resizable==="undefined")c.p.colModel[o].resizable=true;if(c.p.colModel[o].resizable){R=document.createElement("span");a(R).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+j);!a.browser.opera&&a(R).css("cursor","col-resize");a(this).addClass(c.p.resizeclass)}else R="";a(this).css("width",W+"px").prepend(R);c.p.colModel[o].hidden&&a(this).css("display","none");g.headers[o]={width:W,el:this};
...
this)[0].id,s,t,A);return false}});if(c.p.sortable&&a.fn.sortable)try{a(c).jqGrid("sortableColumns",k)}catch(ta){}wa+="</tr></tbody></table>";this.appendChild(document.createElement("tbody"));a(this).addClass("ui-jqgrid-btable");k=a("<table class='ui-jqgrid-htable' style='width:"+c.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'></table>").append(F);var la=c.p.caption&&c.p.hiddengrid===true?true:false;F=a("<div class='ui-jqgrid-hbox"+(j=="rtl"?
"-rtl":"")+"'></div>");g.hDiv=document.createElement("div");a(g.hDiv).css({width:g.width+"px"}).addClass("ui-state-default ui-jqgrid-hdiv").append(F);a(F).append(k);la&&a(g.hDiv).hide();if(c.p.pager){if(typeof c.p.pager=="string"){if(c.p.pager.substr(0,1)!="#")c.p.pager="#"+c.p.pager}else c.p.pager="#"+a(c.p.pager).attr("id");a(c.p.pager).css({width:g.width+"px"}).appendTo(n).addClass("ui-state-default ui-jqgrid-pager ui-corner-bottom");la&&a(c.p.pager).hide();l(c.p.pager,"")}c.p.cellEdit===false&&
...
!s)}}else{if(c.p.multiselect&&c.p.multiboxonly)if(!s){a(c.p.selarrrow).each(function(B,M){B=c.rows.namedItem(M);a(B).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(M),B).attr("checked",false)});c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}a(c).jqGrid("setSelection",K[0].id,true)}if(a.isFunction(c.p.onCellSelect)){ma=K[0].id;oa=a.jgrid.getCellIndex(T);c.p.onCellSelect.call(c,ma,oa,a(T).html(),o)}}o.stopPropagation()}).bind("reloadGrid",function(o,s){if(c.p.treeGrid===
true)c.p.datatype=c.p.treedatatype;s&&s.current&&c.grid.selectionPreserver(c);if(c.p.datatype=="local")a(c).jqGrid("resetSelection");else if(!c.p.treeGrid){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];c.p.scroll&&G(c.grid.bDiv)}if(s&&s.page){o=s.page;if(o>c.p.lastpage)o=c.p.lastpage;if(o<1)o=1;c.p.page=o;c.grid.bDiv.scrollTop=c.grid.prevRowHeight?(o-1)*c.grid.prevRowHeight*c.p.rowNum:0}if(c.grid.prevRowHeight&&
...
d.grid.hDiv).attr("checked",false);d.p.selarrrow=[]}else if(d.p.selrow){a("tr#"+a.jgrid.jqID(d.p.selrow),d.grid.bDiv).removeClass("ui-state-highlight").attr("aria-selected","false");d.p.selrow=null}d.p.savedRow=[]})},getRowData:function(d){var e={},b,f=false,g,h=0;this.each(function(){var j=this,c,k;if(typeof d=="undefined"){f=true;b=[];g=j.rows.length}else{k=j.rows.namedItem(d);if(!k)return e;g=1}for(;h<g;){if(f)k=j.rows[h];a("td",k).each(function(l){c=j.p.colModel[l].name;if(c!=="cb"&&c!=="subgrid")if(j.p.treeGrid===
true&&c==j.p.ExpandColumn)e[c]=a.jgrid.htmlDecode(a("span:first",this).html());else try{e[c]=a.unformat(this,{rowId:k.id,colModel:j.p.colModel[l]},l)}catch(p){e[c]=a.jgrid.htmlDecode(a(this).html())}});h++;if(f){b.push(e);e={}}}});return b?b:e},delRowData:function(d){var e=false,b,f,g;this.each(function(){var h=this;if(b=h.rows.namedItem(d)){g=b.rowIndex;a(b).remove();h.p.records--;h.p.reccount--;h.updatepager(true,false);e=true;if(h.p.multiselect){f=a.inArray(d,h.p.selarrrow);f!=-1&&h.p.selarrrow.splice(f,
1)}if(d==h.p.selrow)h.p.selrow=null}else return false;g==0&&e&&h.updateColumns();if(h.p.altRows===true&&e){var j=h.p.altclass;a(h.rows).each(function(c){c%2==1?a(this).addClass(j):a(this).removeClass(j)})}});return e},setRowData:function(d,e,b){var f,g=false;this.each(function(){var h=this,j,c,k=typeof b;if(!h.grid)return false;c=h.rows.namedItem(d);if(!c)return false;e&&a(this.p.colModel).each(function(l){f=this.name;if(e[f]!=undefined){j=h.formatter(d,e[f],l,e,"edit");h.p.treeGrid===true&&f==h.p.ExpandColumn?
a("td:eq("+l+") > span:first",c).html(j).attr("title",a.jgrid.stripHtml(j)):a("td:eq("+l+")",c).html(j).attr("title",a.jgrid.stripHtml(j));g=true}});if(k==="string")a(c).addClass(b);else k==="object"&&a(c).css(b)});return g},addRowData:function(d,e,b,f){b||(b="last");var g=false,h,j,c,k,l,p,m,n,r="",q,u,v;if(e){if(a.isArray(e)){q=true;b="last";u=d}else{e=[e];q=false}this.each(function(){var w=this,z=e.length;l=w.p.rownumbers===true?1:0;c=w.p.multiselect===true?1:0;k=w.p.subGrid===true?1:0;if(!q)if(typeof d!=
"undefined")d+="";else{d=w.p.records+1+"";if(w.p.keyIndex!==false){cmn=w.p.colModel[w.p.keyIndex+c+k+l].name;if(typeof e[0][cmn]!="undefined")d=e[0][cmn]}}v=w.p.altclass;for(var D=0,C=a.isFunction(w.p.afterInsertRow)?true:false;D<z;){data=e[D];j="";if(q){try{d=data[u]}catch(y){d=w.p.records+1}var G=w.p.altRows===true?(w.rows.length-1)%2==0?v:"":""}if(l){r=w.formatCol(l,1);j+='<td role="gridcell" class="ui-state-default jqgrid-rownum" '+r+">0</td>"}if(c){n='<input type="checkbox" id="jqg_'+d+'" class="cbox"/>';
r=w.formatCol(l,1);j+='<td role="gridcell" '+r+">"+n+"</td>"}if(k)j+=a(w).jqGrid("addSubGridCell",c+l,1);for(m=c+k+l;m<this.p.colModel.length;m++){h=this.p.colModel[m].name;n=w.formatter(d,data[h],m,data,"add");r=w.formatCol(m,1);j+='<td role="gridcell" '+r+' title="'+a.jgrid.stripHtml(n)+'">'+n+"</td>"}j='<tr id="'+d+'" role="row" class="ui-widget-content jqgrow ui-row-'+w.p.direction+" "+G+'">'+j+"</tr>";if(w.p.subGrid===true){j=a(j)[0];a(w).jqGrid("addSubGrid",j,c+l)}if(w.rows.length===0)a("table:first",
...
this,l=a.makeArray(k.cells);a.each(d,function(){var p=l[this];p&&k.appendChild(p)})})}var h=this.get(0);f(h.p.colModel);f(h.p.colNames);f(h.grid.headers);g(a("thead:first",h.grid.hDiv),b&&":not(.ui-jqgrid-labels)");e&&g(a("tbody:first",h.grid.bDiv),".jqgrow");h.p.footerrow&&g(a("tbody:first",h.grid.sDiv));if(h.p.remapColumns)if(h.p.remapColumns.length)f(h.p.remapColumns);else h.p.remapColumns=a.makeArray(d);h.p.lastsort=a.inArray(h.p.lastsort,d);if(h.p.treeGrid)h.p.expColInd=a.inArray(h.p.expColInd,
...
b.p.toolbar[1]=="both"&&a(b.grid.ubDiv).css("width",d+"px")}b.p.footerrow&&a(b.grid.sDiv).css("width",d+"px");if(e===false&&b.p.forceFit==true)b.p.forceFit=false;if(e===true){if(a.browser.safari)h=0;a.each(b.p.colModel,function(){if(this.hidden===false){g+=parseInt(this.width,10);if(this.fixed){n+=this.width;m+=this.width+h}else c++;r++}});if(c!=0){b.p.tblwidth=g;p=d-h*c-m;if(!isNaN(b.p.height))if(a(b.grid.bDiv)[0].clientHeight<a(b.grid.bDiv)[0].scrollHeight){k=true;p-=l}g=0;var u=b.grid.cols.length>
0;a.each(b.p.colModel,function(v){if(this.hidden===false&&!this.fixed){this.width=f=Math.floor(p/(b.p.tblwidth-n)*this.width);g+=f;b.grid.headers[v].width=f;b.grid.headers[v].el.style.width=f+"px";if(b.p.footerrow)b.grid.footers[v].style.width=f+"px";if(u)b.grid.cols[v].style.width=f+"px";j=v}});q=0;if(k){if(d-m-(g+h*c)!==l)q=d-m-(g+h*c)-l}else if(Math.abs(d-m-(g+h*c))!==1)q=d-m-(g+h*c);b.p.colModel[j].width+=q;f=b.p.colModel[j].width;b.grid.headers[j].width=f;b.grid.headers[j].el.style.width=f+"px";
if(u)b.grid.cols[j].style.width=f+"px";b.p.tblwidth=g+q+n+h*r;a("table:first",b.grid.bDiv).css("width",b.p.tblwidth+"px");a("table:first",b.grid.hDiv).css("width",b.p.tblwidth+"px");b.grid.hDiv.scrollLeft=b.grid.bDiv.scrollLeft;if(b.p.footerrow){b.grid.footers[j].style.width=f+"px";a("table:first",b.grid.sDiv).css("width",b.p.tblwidth+"px")}}}}}})},setGridHeight:function(d){return this.each(function(){var e=this;if(e.grid){a(e.grid.bDiv).css({height:d+(isNaN(d)?"":"px")});e.p.height=d;e.p.scroll&&
...
if(a.isFunction(b.p.beforeSubmitCell))(f=b.p.beforeSubmitCell(b.rows[d].id,k,h,d,e))||(f={});if(j=="")j=" ";a("input.hasDatepicker",g).length>0&&a("input.hasDatepicker",g).datepicker("hide");if(b.p.cellsubmit=="remote")if(b.p.cellurl){var q={};if(b.p.autoencode)h=a.jgrid.htmlEncode(h);q[k]=h;m=b.p.prmNames;c=m.id;l=m.oper;q[c]=b.rows[d].id;q[l]=m.editoper;q=a.extend(f,q);a("#lui_"+b.p.id).show();b.grid.hDiv.loading=true;a.ajax(a.extend({url:b.p.cellurl,data:a.isFunction(b.p.serializeCellData)?b.p.serializeCellData(q):
q,type:"POST",complete:function(w,z){a("#lui_"+b.p.id).hide();b.grid.hDiv.loading=false;if(z=="success")if(a.isFunction(b.p.afterSubmitCell)){w=b.p.afterSubmitCell(w,q.id,k,h,d,e);if(w[0]===true){a(g).empty();a(b).jqGrid("setCell",b.rows[d].id,e,j);a(g).addClass("dirty-cell");a(b.rows[d]).addClass("edited");a.isFunction(b.p.afterSaveCell)&&b.p.afterSaveCell(b.rows[d].id,k,h,d,e);b.p.savedRow.splice(0,1)}else{info_dialog(a.jgrid.errors.errcap,w[1],a.jgrid.edit.bClose);a(b).jqGrid("restoreCell",d,e)}}else{a(g).empty();
a(b).jqGrid("setCell",b.rows[d].id,e,j);a(g).addClass("dirty-cell");a(b.rows[d]).addClass("edited");a.isFunction(b.p.afterSaveCell)&&b.p.afterSaveCell(b.rows[d].id,k,h,d,e);b.p.savedRow.splice(0,1)}},error:function(w,z){a("#lui_"+b.p.id).hide();b.grid.hDiv.loading=false;a.isFunction(b.p.errorCell)?b.p.errorCell(w,z):info_dialog(a.jgrid.errors.errcap,w.status+" : "+w.statusText+"<br/>"+z,a.jgrid.edit.bClose);a(b).jqGrid("restoreCell",d,e)}},a.jgrid.ajaxOptions,b.p.ajaxCellOptions||{}))}else try{info_dialog(a.jgrid.errors.errcap,
...
true)){for(var g=e-1;g>=0;g--)if(b.p.colModel[g].editable===true){f=g;break}if(f!==false)a(b).jqGrid("editCell",d,f,true);else b.p.savedRow.length>0&&a(b).jqGrid("saveCell",d,e)}})},GridNav:function(){return this.each(function(){function d(j,c,k){if(k.substr(0,1)=="v"){var l=a(b.grid.bDiv)[0].clientHeight,p=a(b.grid.bDiv)[0].scrollTop,m=b.rows[j].offsetTop+b.rows[j].clientHeight,n=b.rows[j].offsetTop;if(k=="vd")if(m>=l)a(b.grid.bDiv)[0].scrollTop=a(b.grid.bDiv)[0].scrollTop+b.rows[j].clientHeight;
if(k=="vu")if(n<p)a(b.grid.bDiv)[0].scrollTop=a(b.grid.bDiv)[0].scrollTop-b.rows[j].clientHeight}if(k=="h"){k=a(b.grid.bDiv)[0].clientWidth;l=a(b.grid.bDiv)[0].scrollLeft;p=b.rows[j].cells[c].offsetLeft;if(b.rows[j].cells[c].offsetLeft+b.rows[j].cells[c].clientWidth>=k+parseInt(l))a(b.grid.bDiv)[0].scrollLeft=a(b.grid.bDiv)[0].scrollLeft+b.rows[j].cells[c].clientWidth;else if(p<l)a(b.grid.bDiv)[0].scrollLeft=a(b.grid.bDiv)[0].scrollLeft-b.rows[j].cells[c].clientWidth}}function e(j,c){var k,l;if(c==
...
{jqm:g});return false});jQuery(".fm-button","#info_dialog").hover(function(){jQuery(this).addClass("ui-state-hover")},function(){jQuery(this).removeClass("ui-state-hover")});viewModal("#info_dialog",{onHide:function(k){k.w.hide().remove();k.o&&k.o.remove()},modal:f.modal,jqm:g});try{$("#info_dialog").focus()}catch(c){}}function findPos(a){var d=curtop=0;if(a.offsetParent){do{d+=a.offsetLeft;curtop+=a.offsetTop}while(a=a.offsetParent)}return[d,curtop]}
...
q+"]",b).val()){p[q]=n;b.p.marksearched&&a("#jqgh_"+this.name,r.grid.hDiv).addClass("dirty-cell");m++}else{b.p.marksearched&&a("#jqgh_"+this.name,r.grid.hDiv).removeClass("dirty-cell");try{delete r.p.postData[this.index]}catch(z){}}}});var u=m>0?true:false;a.extend(r.p.postData,p);var v;if(b.p.url){v=a(r).jqGrid("getGridParam","url");a(r).jqGrid("setGridParam",{url:b.p.url})}a(r).jqGrid("setGridParam",{search:u}).trigger("reloadGrid",[{page:1}]);v&&a(r).jqGrid("setGridParam",{url:v});a.isFunction(b.p.afterSearch)&&
b.p.afterSearch()},c=function(){var p={},m,n=0,r=a(b.p.gridid)[0],q;a.isFunction(b.p.beforeClear)&&b.p.beforeClear();a.each(b.p.filterModel,function(){q=this.index;m=this.defval?this.defval:"";if(!this.stype)this.stype="text";switch(this.stype){case "select":var w;a("select[name="+q+"] option",b).each(function(C){if(C==0)this.selected=true;if(a(this).text()==m){this.selected=true;w=a(this).val();return false}});if(w){p[q]=w;b.p.marksearched&&a("#jqgh_"+this.name,r.grid.hDiv).addClass("dirty-cell");
n++}else{b.p.marksearched&&a("#jqgh_"+this.name,r.grid.hDiv).removeClass("dirty-cell");try{delete r.p.postData[this.index]}catch(z){}}break;case "text":a("input[name="+q+"]",b).val(m);if(m){p[q]=m;b.p.marksearched&&a("#jqgh_"+this.name,r.grid.hDiv).addClass("dirty-cell");n++}else{b.p.marksearched&&a("#jqgh_"+this.name,r.grid.hDiv).removeClass("dirty-cell");try{delete r.p.postData[this.index]}catch(D){}}break}});var u=n>0?true:false;a.extend(r.p.postData,p);var v;if(b.p.url){v=a(r).jqGrid("getGridParam",
"url");a(r).jqGrid("setGridParam",{url:b.p.url})}a(r).jqGrid("setGridParam",{search:u}).trigger("reloadGrid",[{page:1}]);v&&a(r).jqGrid("setGridParam",{url:v});a.isFunction(b.p.afterClear)&&b.p.afterClear()},k=a("<form name='SearchForm' style=display:inline;' class='"+this.p.formclass+"'></form>"),l=a("<table class='"+this.p.tableclass+"' cellspacing='0' cellpading='0' border='0'><tbody></tbody></table>");a(k).append(l);(function(){var p=document.createElement("tr"),m,n,r,q;b.p.formtype=="horizontal"&&
a(l).append(p);a.each(b.p.filterModel,function(u){r=document.createElement("td");a(r).append("<label for='"+this.name+"'>"+this.label+"</label>");q=document.createElement("td");var v=this;if(!this.stype)this.stype="text";switch(this.stype){case "select":if(this.surl)a(q).load(this.surl,function(){v.defval&&a("select",this).val(v.defval);a("select",this).attr({name:v.index||v.name,id:"sg_"+v.name});v.sopt&&a("select",this).attr(v.sopt);b.p.gridToolbar===true&&v.width&&a("select",this).width(v.width);
...
D.value=C;D.innerHTML=w[C];if(w[C]==v.defval)D.selected="selected";z.appendChild(D)}b.p.gridToolbar===true&&v.width&&a(z).width(v.width);a(q).append(z);b.p.autosearch===true&&a(z).change(function(){j();return false})}break;case "text":z=this.defval?this.defval:"";a(q).append("<input type='text' name='"+(this.index||this.name)+"' id='sg_"+this.name+"' value='"+z+"'/>");v.sopt&&a("input",q).attr(v.sopt);if(b.p.gridToolbar===true&&v.width)a.browser.msie?a("input",q).width(v.width-4):a("input",q).width(v.width-
2);b.p.autosearch===true&&a("input",q).keypress(function(y){if((y.charCode?y.charCode:y.keyCode?y.keyCode:0)==13){j();return false}return this});break}if(b.p.formtype=="horizontal"){b.p.gridToolbar===true&&b.p.gridNames===false?a(p).append(q):a(p).append(r).append(q);a(p).append(q)}else{m=document.createElement("tr");a(m).append(r).append(q);a(l).append(m)}});q=document.createElement("td");if(b.p.enableSearch===true){n="<input type='button' id='sButton' class='"+b.p.buttonclass+"' value='"+b.p.searchButton+
"'/>";a(q).append(n);a("input#sButton",q).click(function(){j();return false})}if(b.p.enableClear===true){n="<input type='button' id='cButton' class='"+b.p.buttonclass+"' value='"+b.p.clearButton+"'/>";a(q).append(n);a("input#cButton",q).click(function(){c();return false})}if(b.p.enableClear===true||b.p.enableSearch===true)if(b.p.formtype=="horizontal")a(p).append(q);else{m=document.createElement("tr");a(m).append("<td>&#160;</td>").append(q);a(l).append(m)}})();a(this).append(k);this.triggerSearch=
...
c=0,k,l,p={};a.each(b.p.colModel,function(){l=this.index||this.name;var v=this.searchoptions&&this.searchoptions.sopt?this.searchoptions.sopt[0]:"bw";switch(this.stype){case "select":if(k=a("select[name="+l+"]",b.grid.hDiv).val()){j[l]=k;p[l]=v;c++}else try{delete b.p.postData[l]}catch(w){}break;case "text":if(k=a("input[name="+l+"]",b.grid.hDiv).val()){j[l]=k;p[l]=v;c++}else try{delete b.p.postData[l]}catch(z){}break}});var m=c>0?true:false;if(d.stringResult){var n='{"groupOp":"'+d.groupOp+'","rules":[',
r=0;a.each(j,function(v,w){if(r>0)n+=",";n+='{"field":"'+v+'",';n+='"op":"'+p[v]+'",';n+='"data":"'+w+'"}';r++});n+="]}";a.extend(b.p.postData,{filters:n})}else a.extend(b.p.postData,j);var q;if(b.p.searchurl){q=b.p.url;a(b).jqGrid("setGridParam",{url:b.p.searchurl})}var u=false;if(a.isFunction(d.beforeSearch))u=d.beforeSearch.call(b);u||a(b).jqGrid("setGridParam",{search:m}).trigger("reloadGrid",[{page:1}]);q&&a(b).jqGrid("setGridParam",{url:q});a.isFunction(d.afterSearch)&&d.afterSearch()},g=a("<tr class='ui-search-toolbar' role='rowheader'></tr>"),
h;a.each(b.p.colModel,function(){var j=this,c,k,l,p;k=a("<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+b.p.direction+"'></th>");c=a("<div style='width:100%;position:relative;height:100%;padding-right:0.3em;'></div>");this.hidden===true&&a(k).css("display","none");this.search=this.search===false?false:true;if(typeof this.stype=="undefined")this.stype="text";l=a.extend({},this.searchoptions||{});if(this.search)switch(this.stype){case "select":if(p=this.surl||l.dataUrl)a.ajax(a.extend({url:p,
dataType:"html",complete:function(u){if(l.buildSelect!=null)(u=l.buildSelect(u))&&a(c).append(u);else a(c).append(u.responseText);l.defaultValue&&a("select",c).val(l.defaultValue);a("select",c).attr({name:j.index||j.name,id:"gs_"+j.name});l.attr&&a("select",c).attr(l.attr);a("select",c).css({width:"100%"});l.dataInit!=null&&l.dataInit(a("select",c)[0]);l.dataEvents!=null&&e(a("select",c)[0],l.dataEvents);d.autosearch===true&&a("select",c).change(function(){f();return false});u=null}},a.jgrid.ajaxOptions,
b.p.ajaxSelectOptions||{}));else{var m;if(j.searchoptions&&j.searchoptions.value)m=j.searchoptions.value;else if(j.editoptions&&j.editoptions.value)m=j.editoptions.value;if(m){p=document.createElement("select");p.style.width="100%";a(p).attr({name:j.index||j.name,id:"gs_"+j.name});if(typeof m==="string"){m=m.split(";");for(var n,r,q=0;q<m.length;q++){n=m[q].split(":");r=document.createElement("option");r.value=n[0];r.innerHTML=n[1];p.appendChild(r)}}else if(typeof m==="object")for(n in m){r=document.createElement("option");
r.value=n;r.innerHTML=m[n];p.appendChild(r)}l.defaultValue&&a(p).val(l.defaultValue);l.attr&&a(p).attr(l.attr);l.dataInit!=null&&l.dataInit(p);l.dataEvents!=null&&e(p,l.dataEvents);a(c).append(p);d.autosearch===true&&a(p).change(function(){f();return false})}}break;case "text":p=l.defaultValue?l.defaultValue:"";a(c).append("<input type='text' style='width:95%;padding:0px;' name='"+(j.index||j.name)+"' id='gs_"+j.name+"' value='"+p+"'/>");l.attr&&a("input",c).attr(l.attr);l.dataInit!=null&&l.dataInit(a("input",
c)[0]);l.dataEvents!=null&&e(a("input",c)[0],l.dataEvents);if(d.autosearch===true)d.searchOnEnter?a("input",c).keypress(function(u){if((u.charCode?u.charCode:u.keyCode?u.keyCode:0)==13){f();return false}return this}):a("input",c).keydown(function(u){switch(u.which){case 9:case 16:case 37:case 38:case 39:case 40:case 27:break;default:h&&clearTimeout(h);h=setTimeout(function(){f()},500)}});break}a(k).append(c);a(g).append(k)});a("table thead",b.grid.hDiv).append(g);this.triggerToolbar=f;this.clearToolbar=
function(j){var c={},k,l=0,p;j=typeof j!="boolean"?true:j;a.each(b.p.colModel,function(){k=this.searchoptions&&this.searchoptions.defaultValue?this.searchoptions.defaultValue:"";p=this.index||this.name;switch(this.stype){case "select":var v;a("select[name="+p+"] option",b.grid.hDiv).each(function(D){if(D==0)this.selected=true;if(a(this).text()==k){this.selected=true;v=a(this).val();return false}});if(v){c[p]=v;l++}else try{delete b.p.postData[p]}catch(w){}break;case "text":a("input[name="+p+"]",b.grid.hDiv).val(k);
if(k){c[p]=k;l++}else try{delete b.p.postData[p]}catch(z){}break}});var m=l>0?true:false;if(d.stringResult){var n='{"groupOp":"'+d.groupOp+'","rules":[',r=0;a.each(c,function(v,w){if(r>0)n+=",";n+='{"field":"'+v+'",';n+='"op":"eq",';n+='"data":"'+w+'"}';r++});n+="]}";a.extend(b.p.postData,{filters:n})}else a.extend(b.p.postData,c);var q;if(b.p.searchurl){q=b.p.url;a(b).jqGrid("setGridParam",{url:b.p.searchurl})}var u=false;if(a.isFunction(d.beforeClear))u=d.beforeClear.call(b);u||j&&a(b).jqGrid("setGridParam",
{search:m}).trigger("reloadGrid",[{page:1}]);q&&a(b).jqGrid("setGridParam",{url:q});a.isFunction(d.afterClear)&&d.afterClear()};this.toggleToolbar=function(){var j=a("tr.ui-search-toolbar",b.grid.hDiv);j.css("display")=="none"?j.show():j.hide()}})}})})(jQuery);
...
this.editoptions.value;if(C){soptions.dataValues=[];if(typeof C==="string"){z=C.split(";");for(n=0;n<z.length;n++){D=z[n].split(":");soptions.dataValues[n]={value:D[0],text:D[1]}}}else if(typeof C==="object"){n=0;for(var y in C){soptions.dataValues[n]={value:y,text:C[y]};n++}}}}if(ignoreHiding&&v||v&&!hidden)k.push(soptions)});if(k.length>0){a("<div id='"+c+"' role='dialog' tabindex='-1'></div>").insertBefore("#gview_"+j.p.id);a("#"+c).searchFilter(k,{groupOps:e.groupOps,operators:u,onClose:g,resetText:e.Reset,
...
closeicon:[],savekey:[false,13],navkeys:[false,38,40],checkOnSubmit:false,checkOnUpdate:false,_savedData:{},processing:false,onClose:null,ajaxEditOptions:{},serializeEditData:null,viewPagerButtons:true},a.jgrid.edit,b||{});return this.each(function(){function f(E,F){E==0?a("#pData","#"+q+"_2").addClass("ui-state-disabled"):a("#pData","#"+q+"_2").removeClass("ui-state-disabled");E==F?a("#nData","#"+q+"_2").addClass("ui-state-disabled"):a("#nData","#"+q+"_2").removeClass("ui-state-disabled")}function g(){var E=
a(m).jqGrid("getDataIDs"),F=a("#id_g","#"+q).val();return[a.inArray(F,E),E]}function h(){var E=true;a("#FormError","#"+q).hide();if(d.checkOnUpdate){x={};N={};j();Y=a.extend({},x,N);if(da=p(Y,d._savedData)){a("#"+r).data("disabled",true);a(".confirm","#"+u.themodal).show();E=false}}return E}function j(){a(".FormElement","#"+q).each(function(){var E=a(".customelement",this);if(E.length){var F=E[0].name;a.each(m.p.colModel,function(){if(this.name==F&&this.editoptions&&a.isFunction(this.editoptions.custom_value)){try{x[F]=
this.editoptions.custom_value(a("#"+F,"#"+q),"get");if(x[F]===undefined)throw"e1";}catch(L){L=="e1"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose):info_dialog(jQuery.jgrid.errors.errcap,L.message,jQuery.jgrid.edit.bClose)}return true}})}else{switch(a(this).get(0).type){case "checkbox":if(a(this).attr("checked"))x[this.name]=a(this).val();else{E=a(this).attr("offval");x[this.name]=E}break;case "select-one":x[this.name]=a("option:selected",
this).val();N[this.name]=a("option:selected",this).text();break;case "select-multiple":x[this.name]=a(this).val();x[this.name]=x[this.name]?x[this.name].join(","):"";var V=[];a("option:selected",this).each(function(L,W){V[L]=a(W).text()});N[this.name]=V.join(",");break;case "password":case "text":case "textarea":case "button":x[this.name]=a(this).val();break}if(m.p.autoencode)x[this.name]=a.jgrid.htmlEncode(x[this.name])}});return true}function c(E,F,V,L){for(var W,R,S,T=0,K,ba,Z,ka=[],aa=false,wa,
ta,la="",ma=1;ma<=L;ma++)la+="<td class='CaptionTD ui-widget-content'>&#160;</td><td class='DataTD ui-widget-content' style='white-space:pre'>&#160;</td>";if(E!="_empty")aa=a(F).jqGrid("getInd",E);a(F.p.colModel).each(function(oa){W=this.name;ba=(R=this.editrules&&this.editrules.edithidden==true?false:this.hidden===true?true:false)?"style='display:none'":"";if(W!=="cb"&&W!=="subgrid"&&this.editable===true&&W!=="rn"){if(aa===false)K="";else if(W==F.p.ExpandColumn&&F.p.treeGrid===true)K=a("td:eq("+
oa+")",F.rows[aa]).text();else try{K=a.unformat(a("td:eq("+oa+")",F.rows[aa]),{rowId:E,colModel:this},oa)}catch(Ia){K=a("td:eq("+oa+")",F.rows[aa]).html()}var xa=a.extend({},this.editoptions||{},{id:W,name:W});frmopt=a.extend({},{elmprefix:"",elmsuffix:"",rowabove:false,rowcontent:""},this.formoptions||{});wa=parseInt(frmopt.rowpos)||T+1;ta=parseInt((parseInt(frmopt.colpos)||1)*2);if(E=="_empty"&&xa.defaultValue)K=a.isFunction(xa.defaultValue)?xa.defaultValue():xa.defaultValue;if(!this.edittype)this.edittype=
"text";if(m.p.autoencode)K=a.jgrid.htmlDecode(K);Z=createEl(this.edittype,xa,K,false,a.extend({},a.jgrid.ajaxOptions,F.p.ajaxSelectOptions||{}));if(K==""&&this.edittype=="checkbox")K=a(Z).attr("offval");if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[W]=K;a(Z).addClass("FormElement");S=a(V).find("tr[rowpos="+wa+"]");if(frmopt.rowabove){xa=a("<tr><td class='contentinfo' colspan='"+L*2+"'>"+frmopt.rowcontent+"</td></tr>");a(V).append(xa);xa[0].rp=wa}if(S.length==0){S=a("<tr "+ba+" rowpos='"+wa+"'></tr>").addClass("FormData").attr("id",
"tr_"+W);a(S).append(la);a(V).append(S);S[0].rp=wa}a("td:eq("+(ta-2)+")",S[0]).html(typeof frmopt.label==="undefined"?F.p.colNames[oa]:frmopt.label);a("td:eq("+(ta-1)+")",S[0]).append(frmopt.elmprefix).append(Z).append(frmopt.elmsuffix);ka[T]=oa;T++}});if(T>0){ma=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(L*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+F.p.id+"_id' value='"+E+"'/></td></tr>");ma[0].rp=T+999;a(V).append(ma);
if(d.checkOnSubmit||d.checkOnUpdate)d._savedData.id=E}return ka}function k(E,F,V){var L,W=0,R,S,T,K,ba;if(d.checkOnSubmit||d.checkOnUpdate){d._savedData={};d._savedData.id=E}var Z=F.p.colModel;if(E=="_empty"){a(Z).each(function(){L=this.name;T=a.extend({},this.editoptions||{});S=a("#"+a.jgrid.jqID(L),"#"+V);if(S[0]!=null){K="";if(T.defaultValue){K=a.isFunction(T.defaultValue)?T.defaultValue():T.defaultValue;if(S[0].type=="checkbox"){ba=K.toLowerCase();if(ba.search(/(false|0|no|off|undefined)/i)<0&&
ba!==""){S[0].checked=true;S[0].defaultChecked=true;S[0].value=K}else S.attr({checked:"",defaultChecked:""})}else S.val(K)}else if(S[0].type=="checkbox"){S[0].checked=false;S[0].defaultChecked=false;K=a(S).attr("offval")}else if(S[0].type.substr(0,6)=="select")S[0].selectedIndex=0;else S.val(K);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[L]=K}});a("#id_g","#"+V).val(E)}else{var ka=a(F).jqGrid("getInd",E,true);if(ka){a("td",ka).each(function(aa){L=Z[aa].name;if(L!=="cb"&&L!=="subgrid"&&
L!=="rn"&&Z[aa].editable===true){if(L==F.p.ExpandColumn&&F.p.treeGrid===true)R=a(this).text();else try{R=a.unformat(this,{rowId:E,colModel:Z[aa]},aa)}catch(wa){R=a(this).html()}if(m.p.autoencode)R=a.jgrid.htmlDecode(R);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[L]=R;L=a.jgrid.jqID(L);switch(Z[aa].edittype){case "password":case "text":case "button":case "image":a("#"+L,"#"+V).val(R);break;case "textarea":if(R=="&nbsp;"||R=="&#160;"||R.length==1&&R.charCodeAt(0)==160)R="";a("#"+L,"#"+V).val(R);
...
function l(){var E,F=[true,"",""],V={},L=m.p.prmNames,W,R;if(a.isFunction(d.beforeCheckValues)){var S=d.beforeCheckValues(x,a("#"+r),x[m.p.id+"_id"]=="_empty"?L.addoper:L.editoper);if(S&&typeof S==="object")x=S}for(var T in x){F=checkValues(x[T],T,m);if(F[0]==false)break}if(F[0]){if(a.isFunction(d.onclickSubmit))V=d.onclickSubmit(d,x)||{};if(a.isFunction(d.beforeSubmit))F=d.beforeSubmit(x,a("#"+r))}G=d.url?d.url:a(m).jqGrid("getGridParam","editurl");if(F[0])if(!G){F[0]=false;F[1]+=" "+a.jgrid.errors.nourl}if(F[0]===
false){a("#FormError>td","#"+q).html(F[1]);a("#FormError","#"+q).show()}else if(!d.processing){d.processing=true;a("#sData","#"+q+"_2").addClass("ui-state-active");R=L.oper;W=L.id;x[R]=a.trim(x[m.p.id+"_id"])=="_empty"?L.addoper:L.editoper;if(x[R]!=L.addoper)x[W]=x[m.p.id+"_id"];else if(x[W]===undefined)x[W]=x[m.p.id+"_id"];delete x[m.p.id+"_id"];x=a.extend(x,d.editData,V);a.ajax(a.extend({url:G,type:d.mtype,data:a.isFunction(d.serializeEditData)?d.serializeEditData(x):x,complete:function(K,ba){if(ba!=
"success"){F[0]=false;F[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(K):ba+" Status: '"+K.statusText+"'. Error code: "+K.status}else if(a.isFunction(d.afterSubmit))F=d.afterSubmit(K,x);if(F[0]===false){a("#FormError>td","#"+q).html(F[1]);a("#FormError","#"+q).show()}else{a.each(m.p.colModel,function(){if(N[this.name]&&this.formatter&&this.formatter=="select")try{delete N[this.name]}catch(ka){}});x=a.extend(x,N);m.p.autoencode&&a.each(x,function(ka,aa){x[ka]=a.jgrid.htmlDecode(aa)});if(x[R]==
L.addoper){F[2]||(F[2]=parseInt(m.p.records)+1);x[W]=F[2];if(d.closeAfterAdd){if(d.reloadAfterSubmit)a(m).trigger("reloadGrid");else{a(m).jqGrid("addRowData",F[2],x,b.addedrow);a(m).jqGrid("setSelection",F[2])}hideModal("#"+u.themodal,{gb:"#gbox_"+n,jqm:b.jqModal,onClose:d.onClose})}else if(d.clearAfterAdd){d.reloadAfterSubmit?a(m).trigger("reloadGrid"):a(m).jqGrid("addRowData",F[2],x,b.addedrow);k("_empty",m,r)}else d.reloadAfterSubmit?a(m).trigger("reloadGrid"):a(m).jqGrid("addRowData",F[2],x,b.addedrow)}else{if(d.reloadAfterSubmit){a(m).trigger("reloadGrid");
...
"#"+q+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+r)[0].focus()}catch(Z){}},error:function(K,ba,Z){a("#FormError>td","#"+q).html(ba+" : "+Z);a("#FormError","#"+q).show();d.processing=false;a("#"+r).data("disabled",false);a("#sData","#"+q+"_2").removeClass("ui-state-active")}},a.jgrid.ajaxOptions,d.ajaxEditOptions))}}function p(E,F){var V=false,L;for(L in E)if(E[L]!=F[L]){V=true;break}return V}var m=this;if(m.grid&&e){var n=m.p.id,r="FrmGrid_"+n,q="TblGrid_"+n,u={themodal:"editmod"+
n,modalhead:"edithd"+n,modalcontent:"editcnt"+n,scrollelm:r},v=a.isFunction(d.beforeShowForm)?d.beforeShowForm:false,w=a.isFunction(d.afterShowForm)?d.afterShowForm:false,z=a.isFunction(d.beforeInitData)?d.beforeInitData:false,D=a.isFunction(d.onInitializeForm)?d.onInitializeForm:false,C=1,y=0,G,x,N,Y,da;if(e=="new"){e="_empty";b.caption=b.addCaption}else b.caption=b.editCaption;b.recreateForm===true&&a("#"+u.themodal).html()!=null&&a("#"+u.themodal).remove();var Q=true;if(b.checkOnUpdate&&b.jqModal&&
!b.modal)Q=false;if(a("#"+u.themodal).html()!=null){a(".ui-jqdialog-title","#"+u.modalhead).html(b.caption);a("#FormError","#"+q).hide();if(d.topinfo){a(".topinfo","#"+q+"_2").html(d.topinfo);a(".tinfo","#"+q+"_2").show()}else a(".tinfo","#"+q+"_2").hide();if(d.bottominfo){a(".bottominfo","#"+q+"_2").html(d.bottominfo);a(".binfo","#"+q+"_2").show()}else a(".binfo","#"+q+"_2").hide();z&&z(a("#"+r));k(e,m,r);e=="_empty"||!d.viewPagerButtons?a("#pData, #nData","#"+q+"_2").hide():a("#pData, #nData","#"+
...
d.bottominfo+"</td></tr>";U+="</tbody></table>";if(y>0){var va=[];a.each(a(ga)[0].rows,function(E,F){va[E]=F});va.sort(function(E,F){if(E.rp>F.rp)return 1;if(E.rp<F.rp)return-1;return 0});a.each(va,function(E,F){a("tbody",ga).append(F)})}b.gbox="#gbox_"+n;var Ga=false;if(b.closeOnEscape===true){b.closeOnEscape=false;Ga=true}ea=a("<span></span>").append(ea).append(U);createModal(u,ea,b,"#gview_"+m.p.id,a("#gview_"+m.p.id)[0]);if(z){a("#pData, #nData","#"+q+"_2").css("float","right");a(".EditButton",
"#"+q+"_2").css("text-align","left")}d.topinfo&&a(".tinfo","#"+q+"_2").show();d.bottominfo&&a(".binfo","#"+q+"_2").show();U=ea=null;a("#"+u.themodal).keydown(function(E){var F=E.target;if(a("#"+r).data("disabled")===true)return false;if(d.savekey[0]===true&&E.which==d.savekey[1])if(F.tagName!="TEXTAREA"){a("#sData","#"+q+"_2").trigger("click");return false}if(E.which===27){if(!h())return false;Ga&&hideModal(this,{gb:b.gbox,jqm:b.jqModal,onClose:d.onClose});return false}if(d.navkeys[0]===true){if(a("#id_g",
...
a("#"+r),E[1][E[0]]);k(E[1][E[0]+1],m,r);a(m).jqGrid("setSelection",E[1][E[0]+1]);a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("next",a("#"+r),E[1][E[0]+1]);f(E[0]+1,E[1].length-1)}return false});a("#pData","#"+q+"_2").click(function(){if(!h())return false;a("#FormError","#"+q).hide();var E=g();if(E[0]!=-1&&E[1][E[0]-1]){a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons("prev",a("#"+r),E[1][E[0]]);k(E[1][E[0]-1],m,r);a(m).jqGrid("setSelection",E[1][E[0]-1]);a.isFunction(b.afterclickPgButtons)&&
...
"#"+n.modalhead).focus()},0)}function g(x,N){x==0?a("#pData","#"+m+"_2").addClass("ui-state-disabled"):a("#pData","#"+m+"_2").removeClass("ui-state-disabled");x==N?a("#nData","#"+m+"_2").addClass("ui-state-disabled"):a("#nData","#"+m+"_2").removeClass("ui-state-disabled")}function h(){var x=a(k).jqGrid("getDataIDs"),N=a("#id_g","#"+m).val();return[a.inArray(N,x),x]}function j(x,N,Y,da){for(var Q,ea,U,ga=0,sa,za,Ca=[],va=false,Ga="<td class='CaptionTD form-view-label ui-widget-content' width='"+b.labelswidth+
"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>",E="",F=["integer","number","currency"],V=0,L=0,W,R,S,T=1;T<=da;T++)E+=T==1?Ga:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";a(N.p.colModel).each(function(){ea=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false;if(!ea&&this.align==="right")if(this.formatter&&a.inArray(this.formatter,F)!==
...
m).html(ea);da&&a("#"+Y,"#"+m).parents("tr:first").hide();Q++}});Q>0&&a("#id_g","#"+m).val(x)}}var k=this;if(k.grid&&e){if(!b.imgpath)b.imgpath=k.p.imgpath;var l=k.p.id,p="ViewGrid_"+l,m="ViewTbl_"+l,n={themodal:"viewmod"+l,modalhead:"viewhd"+l,modalcontent:"viewcnt"+l,scrollelm:p},r=1,q=0;if(a("#"+n.themodal).html()!=null){a(".ui-jqdialog-title","#"+n.modalhead).html(b.caption);a("#FormError","#"+m).hide();c(e,k);a.isFunction(b.beforeShowForm)&&b.beforeShowForm(a("#"+p));viewModal("#"+n.themodal,
...
a(v).append(w);j(e,k,w,r);u=k.p.direction=="rtl"?true:false;var z="<a href='javascript:void(0)' id='"+(u?"nData":"pData")+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></div>",D="<a href='javascript:void(0)' id='"+(u?"pData":"nData")+"' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></div>",C="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"+b.bClose+
"</a>";if(q>0){var y=[];a.each(a(w)[0].rows,function(x,N){y[x]=N});y.sort(function(x,N){if(x.rp>N.rp)return 1;if(x.rp<N.rp)return-1;return 0});a.each(y,function(x,N){a("tbody",w).append(N)})}b.gbox="#gbox_"+l;var G=false;if(b.closeOnEscape===true){b.closeOnEscape=false;G=true}v=a("<span></span>").append(v).append("<table border='0' class='EditTable' id='"+m+"_2'><tbody><tr id='Act_Buttons'><td class='navButton ui-widget-content' width='"+b.labelswidth+"'>"+(u?D+z:z+D)+"</td><td class='EditButton ui-widget-content'>"+
C+"</td></tr></tbody></table>");createModal(n,v,b,"#gview_"+k.p.id,a("#gview_"+k.p.id)[0]);if(u){a("#pData, #nData","#"+m+"_2").css("float","right");a(".EditButton","#"+m+"_2").css("text-align","left")}b.viewPagerButtons||a("#pData, #nData","#"+m+"_2").hide();v=null;a("#"+n.themodal).keydown(function(x){if(x.which===27){G&&hideModal(this,{gb:b.gbox,jqm:b.jqModal,onClose:b.onClose});return false}if(b.navkeys[0]===true){if(x.which===b.navkeys[1]){a("#pData","#"+m+"_2").trigger("click");return false}if(x.which===
b.navkeys[2]){a("#nData","#"+m+"_2").trigger("click");return false}}});b.closeicon=a.extend([true,"left","ui-icon-close"],b.closeicon);if(b.closeicon[0]==true)a("#cData","#"+m+"_2").addClass(b.closeicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.closeicon[2]+"'></span>");a.isFunction(b.beforeShowForm)&&b.beforeShowForm(a("#"+p));viewModal("#"+n.themodal,{gbox:"#gbox_"+l,jqm:b.jqModal,modal:b.modal});a(".fm-button:not(.ui-state-disabled)","#"+m+"_2").hover(function(){a(this).addClass("ui-state-hover")},
function(){a(this).removeClass("ui-state-hover")});f();a("#cData","#"+m+"_2").click(function(){hideModal("#"+n.themodal,{gb:"#gbox_"+l,jqm:b.jqModal,onClose:b.onClose});return false});a("#nData","#"+m+"_2").click(function(){a("#FormError","#"+m).hide();var x=h();x[0]=parseInt(x[0]);if(x[0]!=-1&&x[1][x[0]+1]){a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons("next",a("#"+p),x[1][x[0]]);c(x[1][x[0]+1],k);a(k).jqGrid("setSelection",x[1][x[0]+1]);a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("next",
a("#"+p),x[1][x[0]+1]);g(x[0]+1,x[1].length-1)}f();return false});a("#pData","#"+m+"_2").click(function(){a("#FormError","#"+m).hide();var x=h();if(x[0]!=-1&&x[1][x[0]-1]){a.isFunction(b.onclickPgButtons)&&b.onclickPgButtons("prev",a("#"+p),x[1][x[0]]);c(x[1][x[0]-1],k);a(k).jqGrid("setSelection",x[1][x[0]-1]);a.isFunction(b.afterclickPgButtons)&&b.afterclickPgButtons("prev",a("#"+p),x[1][x[0]-1]);g(x[0]-1,x[1].length-1)}f();return false})}u=h();g(u[0],u[1].length-1)}})},delGridRow:function(e,b){d=
...
h=typeof b.afterShowForm==="function"?true:false,j=f.p.id,c={},k="DelTbl_"+j,l,p,m,n,r={themodal:"delmod"+j,modalhead:"delhd"+j,modalcontent:"delcnt"+j,scrollelm:k};if(isArray(e))e=e.join();if(a("#"+r.themodal).html()!=null){a("#DelData>td","#"+k).text(e);a("#DelError","#"+k).hide();if(d.processing===true){d.processing=false;a("#dData","#"+k).removeClass("ui-state-active")}g&&b.beforeShowForm(a("#"+k));viewModal("#"+r.themodal,{gbox:"#gbox_"+j,jqm:b.jqModal,jqM:false,modal:b.modal})}else{var q=isNaN(b.dataheight)?
b.dataheight:b.dataheight+"px";q="<div id='"+k+"' class='formdata' style='width:100%;overflow:auto;position:relative;height:"+q+";'>";q+="<table class='DelTable'><tbody>";q+="<tr id='DelError' style='display:none'><td class='ui-state-error'></td></tr>";q+="<tr id='DelData' style='display:none'><td >"+e+"</td></tr>";q+='<tr><td class="delmsg" style="white-space:pre;">'+b.msg+"</td></tr><tr><td >&#160;</td></tr>";q+="</tbody></table></div>";q+="<table cellspacing='0' cellpadding='0' border='0' class='EditTable' id='"+
...
"#"+k).show()}else if(!d.processing){d.processing=true;a(this).addClass("ui-state-active");m=f.p.prmNames;l=a.extend({},d.delData,c);n=m.oper;l[n]=m.deloper;p=m.id;l[p]=v;a.ajax(a.extend({url:w,type:b.mtype,data:a.isFunction(b.serializeDelData)?b.serializeDelData(l):l,complete:function(z,D){if(D!="success"){u[0]=false;u[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(z):D+" Status: '"+z.statusText+"'. Error code: "+z.status}else if(typeof d.afterSubmit==="function")u=d.afterSubmit(z,l);if(u[0]===
false){a("#DelError>td","#"+k).html(u[1]);a("#DelError","#"+k).show()}else{if(d.reloadAfterSubmit)a(f).trigger("reloadGrid");else{D=[];D=v.split(",");if(f.p.treeGrid===true)try{a(f).jqGrid("delTreeNode",D[0])}catch(C){}else for(var y=0;y<D.length;y++)a(f).jqGrid("delRowData",D[y]);f.p.selrow=null;f.p.selarrrow=[]}a.isFunction(d.afterComplete)&&setTimeout(function(){d.afterComplete(z,v)},500)}d.processing=false;a("#dData","#"+k+"_2").removeClass("ui-state-active");u[0]&&hideModal("#"+r.themodal,{gb:"#gbox_"+
j,jqm:b.jqModal,onClose:d.onClose})},error:function(z,D,C){a("#DelError>td","#"+k).html(D+" : "+C);a("#DelError","#"+k).show();d.processing=false;a("#dData","#"+k+"_2").removeClass("ui-state-active")}},a.jgrid.ajaxOptions,b.ajaxDelOptions))}return false});a("#eData","#"+k+"_2").click(function(){hideModal("#"+r.themodal,{gb:"#gbox_"+j,jqm:b.jqModal,onClose:d.onClose});return false});g&&b.beforeShowForm(a("#"+k));viewModal("#"+r.themodal,{gbox:"#gbox_"+j,jqm:b.jqModal,modal:b.modal})}h&&b.afterShowForm(a("#"+
...
b||{});return this.each(function(){var k={themodal:"alertmod",modalhead:"alerthd",modalcontent:"alertcnt"},l=this,p,m,n;if(!(!l.grid||typeof e!="string")){if(a("#"+k.themodal).html()==null){if(typeof window.innerWidth!="undefined"){p=window.innerWidth;m=window.innerHeight}else if(typeof document.documentElement!="undefined"&&typeof document.documentElement.clientWidth!="undefined"&&document.documentElement.clientWidth!=0){p=document.documentElement.clientWidth;m=document.documentElement.clientHeight}else{p=
1024;m=768}createModal(k,"<div>"+b.alerttext+"</div><span tabindex='0'><span tabindex='-1' id='jqg_alrt'></span></span>",{gbox:"#gbox_"+l.p.id,jqModal:true,drag:true,resize:true,caption:b.alertcap,top:m/2-25,left:p/2-100,width:200,height:"auto",closeOnEscape:b.closeOnEscape},"","",true)}p=1;if(b.cloneToTop&&l.p.toppager)p=2;for(m=0;m<p;m++){var r=a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"),
q,u;if(m==0){q=e;u=l.p.id;if(q==l.p.toppager){u+="_top";p=1}}else{q=l.p.toppager;u=l.p.id+"_top"}l.p.direction=="rtl"&&a(r).attr("dir","rtl").css("float","right");if(b.add){g=g||{};n=a("<td class='ui-pg-button ui-corner-all'></td>");a(n).append("<div class='ui-pg-div'><span class='ui-icon "+b.addicon+"'></span>"+b.addtext+"</div>");a("tr",r).append(n);a(n,r).attr({title:b.addtitle||"",id:g.id||"add_"+u}).click(function(){typeof b.addfunc=="function"?b.addfunc():a(l).jqGrid("editGridRow","new",g);
return false}).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});n=null}if(b.edit){n=a("<td class='ui-pg-button ui-corner-all'></td>");f=f||{};a(n).append("<div class='ui-pg-div'><span class='ui-icon "+b.editicon+"'></span>"+b.edittext+"</div>");a("tr",r).append(n);a(n,r).attr({title:b.edittitle||"",id:f.id||"edit_"+u}).click(function(){var v=l.p.selrow;if(v)typeof b.editfunc=="function"?b.editfunc(v):a(l).jqGrid("editGridRow",v,f);else{viewModal("#"+
k.themodal,{gbox:"#gbox_"+l.p.id,jqm:true});a("#jqg_alrt").focus()}return false}).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});n=null}if(b.view){n=a("<td class='ui-pg-button ui-corner-all'></td>");c=c||{};a(n).append("<div class='ui-pg-div'><span class='ui-icon "+b.viewicon+"'></span>"+b.viewtext+"</div>");a("tr",r).append(n);a(n,r).attr({title:b.viewtitle||"",id:c.id||"view_"+u}).click(function(){var v=l.p.selrow;if(v)a(l).jqGrid("viewGridRow",
v,c);else{viewModal("#"+k.themodal,{gbox:"#gbox_"+l.p.id,jqm:true});a("#jqg_alrt").focus()}return false}).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});n=null}if(b.del){n=a("<td class='ui-pg-button ui-corner-all'></td>");h=h||{};a(n).append("<div class='ui-pg-div'><span class='ui-icon "+b.delicon+"'></span>"+b.deltext+"</div>");a("tr",r).append(n);a(n,r).attr({title:b.deltitle||"",id:h.id||"del_"+u}).click(function(){var v;if(l.p.multiselect){v=
l.p.selarrrow;if(v.length==0)v=null}else v=l.p.selrow;if(v)a(l).jqGrid("delGridRow",v,h);else{viewModal("#"+k.themodal,{gbox:"#gbox_"+l.p.id,jqm:true});a("#jqg_alrt").focus()}return false}).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});n=null}if(b.add||b.edit||b.del||b.view)a("tr",r).append("<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>");if(b.search){n=a("<td class='ui-pg-button ui-corner-all'></td>");
j=j||{};a(n).append("<div class='ui-pg-div'><span class='ui-icon "+b.searchicon+"'></span>"+b.searchtext+"</div>");a("tr",r).append(n);a(n,r).attr({title:b.searchtitle||"",id:j.id||"search_"+u}).click(function(){a(l).jqGrid("searchGrid",j);return false}).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});n=null}if(b.refresh){n=a("<td class='ui-pg-button ui-corner-all'></td>");a(n).append("<div class='ui-pg-div'><span class='ui-icon "+b.refreshicon+
"'></span>"+b.refreshtext+"</div>");a("tr",r).append(n);a(n,r).attr({title:b.refreshtitle||"",id:"refresh_"+u}).click(function(){a.isFunction(b.beforeRefresh)&&b.beforeRefresh();l.p.search=false;try{a("#fbox_"+l.p.id).searchFilter().reset();l.clearToolbar(false)}catch(v){}switch(b.refreshstate){case "firstpage":a(l).trigger("reloadGrid",[{page:1}]);break;case "current":a(l).trigger("reloadGrid",[{current:true}]);break}a.isFunction(b.afterRefresh)&&b.afterRefresh();return false}).hover(function(){a(this).addClass("ui-state-hover")},
function(){a(this).removeClass("ui-state-hover")});n=null}n=a(".ui-jqgrid").css("font-size")||"11px";a("body").append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+n+";visibility:hidden;' ></div>");n=a(r).clone().appendTo("#testpg2").width();a("#testpg2").remove();a(q+"_"+b.position,q).append(r);if(l.p._nvtd){if(n>l.p._nvtd[0]){a(q+"_"+b.position,q).width(n);l.p._nvtd[0]=n}l.p._nvtd[1]=n}r=n=n=null}}})},navButtonAdd:function(e,b){b=a.extend({caption:"newButton",
...
(f="set");g||(g="first");var j=a(b).serializeArray(),c={};a.each(j,function(k,l){c[l.name]=l.value});if(f=="add")a(h).jqGrid("addRowData",e,c,g);else f=="set"&&a(h).jqGrid("setRowData",e,c)}})}})})(jQuery);
...
(function(a){a.jgrid.extend({editRow:function(d,e,b,f,g,h,j,c,k){return this.each(function(){var l=this,p,m,n=0,r=null,q={},u,v;if(l.grid){u=a(l).jqGrid("getInd",d,true);if(u!=false)if((a(u).attr("editable")||"0")=="0"&&!a(u).hasClass("not-editable-row")){v=l.p.colModel;a("td",u).each(function(w){p=v[w].name;var z=l.p.treeGrid===true&&p==l.p.ExpandColumn;if(z)m=a("span:first",this).html();else try{m=a.unformat(this,{rowId:d,colModel:v[w]},w)}catch(D){m=a(this).html()}if(p!="cb"&&p!="subgrid"&&p!=
"rn"){if(l.p.autoencode)m=a.jgrid.htmlDecode(m);q[p]=m;if(v[w].editable===true){if(r===null)r=w;z?a("span:first",this).html(""):a(this).html("");var C=a.extend({},v[w].editoptions||{},{id:d+"_"+p,name:p});if(!v[w].edittype)v[w].edittype="text";C=createEl(v[w].edittype,C,m,true,a.extend({},a.jgrid.ajaxOptions,l.p.ajaxSelectOptions||{}));a(C).addClass("editable");z?a("span:first",this).append(C):a(this).append(C);v[w].edittype=="select"&&v[w].editoptions.multiple===true&&a.browser.msie&&a(C).width(a(C).width());
n++}}});if(n>0){q.id=d;l.p.savedRow.push(q);a(u).attr("editable","1");a("td:eq("+r+") input",u).focus();e===true&&a(u).bind("keydown",function(w){w.keyCode===27&&a(l).jqGrid("restoreRow",d,k);if(w.keyCode===13){if(w.target.tagName=="TEXTAREA")return true;a(l).jqGrid("saveRow",d,f,g,h,j,c,k);return false}w.stopPropagation()});a.isFunction(b)&&b(d)}}}})},saveRow:function(d,e,b,f,g,h,j){return this.each(function(){var c=this,k,l={},p={},m,n,r,q;if(c.grid){q=a(c).jqGrid("getInd",d,true);if(q!=false){m=
a(q).attr("editable");b=b?b:c.p.editurl;if(m==="1"&&b){var u;a("td",q).each(function(D){u=c.p.colModel[D];k=u.name;if(k!="cb"&&k!="subgrid"&&u.editable===true&&k!="rn"){switch(u.edittype){case "checkbox":var C=["Yes","No"];if(u.editoptions)C=u.editoptions.value.split(":");l[k]=a("input",this).attr("checked")?C[0]:C[1];break;case "text":case "password":case "textarea":case "button":l[k]=a("input, textarea",this).val();break;case "select":if(u.editoptions.multiple){C=a("select",this);var y=[];l[k]=
a(C).val();l[k]=l[k]?l[k].join(","):"";a("select > option:selected",this).each(function(x,N){y[x]=a(N).text()});p[k]=y.join(",")}else{l[k]=a("select>option:selected",this).val();p[k]=a("select>option:selected",this).text()}if(u.formatter&&u.formatter=="select")p={};break;case "custom":try{if(u.editoptions&&a.isFunction(u.editoptions.custom_value)){l[k]=u.editoptions.custom_value(a(".customelement",this),"get");if(l[k]===undefined)throw"e2";}else throw"e1";}catch(G){G=="e1"&&info_dialog(jQuery.jgrid.errors.errcap,
"function 'custom_value' "+a.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose);G=="e2"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose):info_dialog(jQuery.jgrid.errors.errcap,G.message,jQuery.jgrid.edit.bClose)}break}r=checkValues(l[k],D,c);if(r[0]===false){r[1]=l[k]+" "+r[1];return false}if(c.p.autoencode)l[k]=a.jgrid.htmlEncode(l[k])}});if(r[0]===false)try{var v=findPos(a("#"+d)[0]);info_dialog(a.jgrid.errors.errcap,r[1],a.jgrid.edit.bClose,
{left:v[0],top:v[1]})}catch(w){alert(r[1])}else{if(l){var z;v=c.p.prmNames;z=v.oper;m=v.id;l[z]=v.editoper;l[m]=d;if(f)l=a.extend({},l,f)}if(b=="clientArray"){l=a.extend({},l,p);c.p.autoencode&&a.each(l,function(D,C){l[D]=a.jgrid.htmlDecode(C)});m=a(c).jqGrid("setRowData",d,l);a(q).attr("editable","0");for(v=0;v<c.p.savedRow.length;v++)if(c.p.savedRow[v].id==d){n=v;break}n>=0&&c.p.savedRow.splice(n,1);a.isFunction(g)&&g(d,m)}else{a("#lui_"+c.p.id).show();a.ajax(a.extend({url:b,data:a.isFunction(c.p.serializeRowData)?
c.p.serializeRowData(l):l,type:"POST",complete:function(D,C){a("#lui_"+c.p.id).hide();if(C==="success")if((a.isFunction(e)?e(D):true)===true){c.p.autoencode&&a.each(l,function(y,G){l[y]=a.jgrid.htmlDecode(G)});l=a.extend({},l,p);a(c).jqGrid("setRowData",d,l);a(q).attr("editable","0");for(C=0;C<c.p.savedRow.length;C++)if(c.p.savedRow[C].id==d){n=C;break}n>=0&&c.p.savedRow.splice(n,1);a.isFunction(g)&&g(d,D)}else a(c).jqGrid("restoreRow",d,j)},error:function(D,C){a("#lui_"+c.p.id).hide();a.isFunction(h)?
...
k=a("#"+f.id).jqGrid("getRowData",k);if(!h.dropbyname){var l=0,p={},m,n=a("#"+this.id).jqGrid("getGridParam","colModel");try{for(key in k){if(n[l]){m=n[l].name;p[m]=k[key]}l++}k=p}catch(r){}}c.helper.dropped=true;if(h.beforedrop&&a.isFunction(h.beforedrop)){m=h.beforedrop.call(this,j,c,k,a("#"+f.id),a(this));if(typeof m!="undefined"&&m!==null&&typeof m=="object")k=m}if(c.helper.dropped){var q;if(h.autoid)if(a.isFunction(h.autoid))q=h.autoid.call(this,k);else{q=Math.ceil(Math.random()*1E3);q=h.autoidprefix+
q}a("#"+this.id).jqGrid("addRowData",q,k,h.droppos)}h.ondrop&&a.isFunction(h.ondrop)&&h.ondrop.call(this,j,c,k)}},h.drop_opts||{})},onstart:null,onstop:null,beforedrop:null,ondrop:null,drop_opts:{activeClass:"ui-state-active",hoverClass:"ui-state-hover"},drag_opts:{revert:"invalid",helper:"clone",cursor:"move",appendTo:"#jqgrid_dnd",zIndex:5E3},dropbyname:false,droppos:"first",autoid:true,autoidprefix:"dnd_"},e||{});if(e.connectWith){e.connectWith=e.connectWith.split(",");e.connectWith=a.map(e.connectWith,
...
var p=function(q){var u,v,w,z;u=a(q).attr("id");v={nd_:(new Date).getTime()};v[b.p.idName]=u;if(!b.p.subGridModel[0])return false;if(b.p.subGridModel[0].params)for(z=0;z<b.p.subGridModel[0].params.length;z++)for(w=0;w<b.p.colModel.length;w++)if(b.p.colModel[w].name==b.p.subGridModel[0].params[z])v[b.p.colModel[w].name]=a("td:eq("+w+")",q).text().replace(/\&#160\;/ig,"");if(!b.grid.hDiv.loading){b.grid.hDiv.loading=true;a("#load_"+b.p.id).show();if(!b.p.subgridtype)b.p.subgridtype=b.p.datatype;if(a.isFunction(b.p.subgridtype))b.p.subgridtype(v);
...
jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this).html())).split(" ").join("_"),index:jQuery(this).attr("id")||jQuery.trim(jQuery.jgrid.stripHtml(jQuery(this).html())).split(" ").join("_"),width:jQuery(this).width()||150});k.push(jQuery(this).html())}});var l=[],p=[],m=[];jQuery("tbody > tr",jQuery(this)).each(function(){var n={},r=0;jQuery("td",jQuery(this)).each(function(){if(r==0&&j){var q=jQuery("input",jQuery(this)),u=q.attr("value");p.push(u||l.length);q.attr("checked")&&m.push(u);n[c[r].name]=
q.attr("value")}else n[c[r].name]=jQuery(this).html();r++});r>0&&l.push(n)});jQuery(this).empty();jQuery(this).addClass("scroll");jQuery(this).jqGrid($.extend({datatype:"local",width:e,colNames:k,colModel:c,multiselect:g},d||{}));for(e=0;e<l.length;e++){g=null;if(p.length>0)if((g=p[e])&&g.replace)g=encodeURIComponent(g).replace(/[.\-%]/g,"_");if(g==null)g=e+1;jQuery(this).jqGrid("addRowData",g,l[e])}for(e=0;e<m.length;e++)jQuery(this).jqGrid("setSelection",m[e])}})}
...
b=d.e,f,g=function(j,c,k,l){return j.each(function(){c=c?a(c,j):j;c.bind("mousedown",{e:j,k:k},function(p){var m=p.data,n={};b=m.e;f=l?a(l):false;if(b.css("position")!="relative")try{b.position(n)}catch(r){}e={X:n.left||h("left")||0,Y:n.top||h("top")||0,W:h("width")||b[0].scrollWidth||0,H:h("height")||b[0].scrollHeight||0,pX:p.pageX,pY:p.pageY,k:m.k};M1=f&&m.k!="d"?{X:n.left||f1("left")||0,Y:n.top||f1("top")||0,W:f[0].offsetWidth||f1("width")||0,H:f[0].offsetHeight||f1("height")||0,pX:p.pageX,pY:p.pageY,
...
0,e=a.jqm.hash,b=[],f=a.browser.msie&&a.browser.version=="6.0",g=false,h=function(p){var m=a('<iframe src="javascript:false;document.write(\'\');" class="jqm"></iframe>').css({opacity:0});if(f)if(p.o)p.o.html('<p style="width:100%;height:100%"/>').prepend(m);else a("iframe.jqm",p.w)[0]||p.w.prepend(m);j(p)},j=function(p){try{a(":input:visible",p.w)[0].focus()}catch(m){}},c=function(p){a(document)[p]("keypress",k)[p]("keydown",k)[p]("mousedown",k)},k=function(p){var m=e[b[b.length-1]];(p=!a(p.target).parents(".jqmID"+
m.s)[0])&&j(m);return!p},l=function(p,m,n){return p.each(function(){var r=this._jqm;a(m).each(function(){if(!this[n]){this[n]=[];a(this).click(function(){for(var q in{jqmShow:1,jqmHide:1})for(var u in this[q])e[this[q][u]]&&e[this[q][u]].w[q](this);return g})}this[n].push(r)})})}})(jQuery);
...
g.replace(".",h);for(;g.length-1-e<j;)g+="0"}}if(b.thousandsSeparator){j=b.thousandsSeparator;e=g.lastIndexOf(h);e=e>-1?e:g.length;h=g.substring(e);for(var c=-1,k=e;k>0;k--){c++;if(c%3===0&&k!==e&&(!f||k>1))h=j+h;h=g.charAt(k-1)+h}g=h}g=b.prefix?b.prefix+g:g;return g=b.suffix?g+b.suffix:g}else return e},DateFormat:function(e,b,f,g){var h=function(z,D){z=String(z);for(D=parseInt(D)||2;z.length<D;)z="0"+z;return z},j={m:1,d:1,y:1970,h:0,i:0,s:0},c=0,k,l,p=["i18n"];p.i18n={dayNames:g.dayNames,monthNames:g.monthNames};
if(e in g.masks)e=g.masks[e];b=b.split(/[\\\/:_;.\t\T\s-]/);e=e.split(/[\\\/:_;.\t\T\s-]/);k=0;for(l=e.length;k<l;k++){if(e[k]=="M"){c=a.inArray(b[k],p.i18n.monthNames);if(c!==-1&&c<12)b[k]=c+1}if(e[k]=="F"){c=a.inArray(b[k],p.i18n.monthNames);if(c!==-1&&c>11)b[k]=c+1-12}j[e[k].toLowerCase()]=parseInt(b[k],10)}j.m=parseInt(j.m)-1;c=j.y;if(c>=70&&c<=99)j.y=1900+j.y;else if(c>=0&&c<=69)j.y=2E3+j.y;c=new Date(j.y,j.m,j.d,j.h,j.i,j.s,0);if(f in g.masks)f=g.masks[f];else f||(f="Y-m-d");j=c.getHours();
e=c.getMinutes();b=c.getDate();k=c.getMonth()+1;l=c.getTimezoneOffset();var m=c.getSeconds(),n=c.getMilliseconds(),r=c.getDay(),q=c.getFullYear(),u=(r+6)%7+1,v=(new Date(q,k-1,b)-new Date(q,0,1))/864E5,w={d:h(b),D:p.i18n.dayNames[r],j:b,l:p.i18n.dayNames[r+7],N:u,S:g.S(b),w:r,z:v,W:u<5?Math.floor((v+u-1)/7)+1:Math.floor((v+u-1)/7)||(((new Date(q-1,0,1)).getDay()+6)%7<4?53:52),F:p.i18n.monthNames[k-1+12],m:h(k),M:p.i18n.monthNames[k-1],n:k,t:"?",L:"?",o:"?",Y:q,y:String(q).substring(2),a:j<12?g.AmPm[0]:
g.AmPm[1],A:j<12?g.AmPm[2]:g.AmPm[3],B:"?",g:j%12||12,G:j,h:h(j%12||12),H:h(j),i:h(e),s:h(m),u:n,e:"?",I:"?",O:(l>0?"-":"+")+h(Math.floor(Math.abs(l)/60)*100+Math.abs(l)%60,4),P:"?",T:(String(c).match(/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g)||[""]).pop().replace(/[^-+\dA-Z]/g,""),Z:"?",c:"?",r:"?",U:Math.floor(c/1E3)};return f.replace(/\\.|[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU]/g,function(z){return z in
...
j=a.map(j,function(m){return a.trim(m)})}if(isString(f))for(var k=f.split(";"),l=0,p=0;p<k.length;p++){c=k[p].split(":");if(c.length>2)c[1]=jQuery.map(c,function(m,n){if(n>0)return m}).join(":");if(h){if(jQuery.inArray(c[0],j)>-1){g[l]=c[1];l++}}else if(a.trim(c[0])==a.trim(e)){g[0]=c[1];break}}else if(isObject(f))if(h)g=jQuery.map(j,function(m){return f[m]});else g[0]=f[e]||""}e=g.join(", ");return e==""?a.fn.fmatter.defaultFormat(e,b):e};a.fn.fmatter.rowactions=function(e,b,f,g){switch(f){case "edit":f=
...
b=a.extend({},b.colModel.editoptions);if(b.value){var h=b.value;b=b.multiple===true?true:false;g=[];var j;if(b){g=e.split(",");g=a.map(g,function(p){return a.trim(p)})}if(isString(h))for(var c=h.split(";"),k=0,l=0;l<c.length;l++){j=c[l].split(":");if(b){if(jQuery.inArray(j[1],g)>-1){f[k]=j[0];k++}}else if(a.trim(j[1])==a.trim(e)){f[0]=j[0];break}}else if(isObject(h)){b||(g[0]=e);f=jQuery.map(g,function(p){var m;a.each(h,function(n,r){if(r==p){m=n;return false}});if(m)return m})}return f.join(", ")}else return e||
...
return false}function j(y){jQuery(this).toggleClass("ui-state-active",y.type=="mousedown");return false}function c(y,G){return"<option value='"+y+"'>"+G+"</option>"}function k(y,G,x){return"<select class='"+y+"'"+(x?" style='display:none;'":"")+">"+G+"</select>"}function l(y,G){y=b.find("tr.sf td.data "+y);y[0]!=null&&G(y)}function p(y,G){var x=b.find("tr.sf td.data "+y);x[0]!=null&&jQuery.each(G,function(){this.data!=null?x.bind(this.type,this.data,this.fn):x.bind(this.type,this.fn)})}var m=jQuery.extend({},
...
g=b.find("tr.sf");var q=g.find("td.fields"),u=g.find("td.ops"),v=g.find("td.data"),w="";jQuery.each(m.operators,function(){w+=c(this.op,this.text)});w=k("default",w,true);u.append(w);v.append("<input type='text' class='default' style='display:none;' />");var z="",D=false,C=false;jQuery.each(f,function(y){z+=c(this.itemval,this.text);if(this.ops!=null){D=true;var G="";jQuery.each(this.ops,function(){G+=c(this.op,this.text)});G=k("field"+y,G,true);u.append(G)}if(this.dataUrl!=null){if(y>n)n=y;C=true;
var x=this.dataEvents,N=this.dataInit,Y=this.buildSelect;jQuery.ajax(jQuery.extend({url:this.dataUrl,complete:function(Q){Q=Y!=null?jQuery("<div />").append(Y(Q)):jQuery("<div />").append(Q.responseText);Q.find("select").addClass("field"+y).hide();v.append(Q.html());N&&l(".field"+y,N);x&&p(".field"+y,x);y==n&&b.find("tr.sf td.fields select[name='field']").change()}},m.ajaxSelectOptions))}else if(this.dataValues!=null){C=true;var da="";jQuery.each(this.dataValues,function(){da+=c(this.value,this.text)});
da=k("field"+y,da,true);v.append(da)}else if(this.dataEvents!=null||this.dataInit!=null){C=true;da="<input type='text' class='field"+y+"' />";v.append(da)}this.dataInit!=null&&y!=n&&l(".field"+y,this.dataInit);this.dataEvents!=null&&y!=n&&p(".field"+y,this.dataEvents)});z="<select name='field'>"+z+"</select>";q.append(z);f=q.find("select[name='field']");D?f.change(function(y){var G=y.target.selectedIndex;y=jQuery(y.target).parents("tr.sf").find("td.ops");y.find("select").removeAttr("name").hide();
G=y.find(".field"+G);if(G[0]==null)G=y.find(".default");G.attr("name","op").show()}):u.find(".default").attr("name","op").show();C?f.change(function(y){var G=y.target.selectedIndex;y=jQuery(y.target).parents("tr.sf").find("td.data");y.find("select,input").removeClass("vdata").hide();G=y.find(".field"+G);if(G[0]==null)G=y.find(".default");G.show().addClass("vdata")}):v.find(".default").show().addClass("vdata");if(D||C)f.change();b.find(".ui-state-default").hover(h,h).mousedown(j).mouseup(j);b.find(".ui-closer").click(function(){m.onClose(jQuery(b.selector));
...
jQuery(y.target).parents(".sf");var G=y.clone(true).insertAfter(y);G.find(".ui-state-default").removeClass("ui-state-hover ui-state-active");if(m.clone){G.find("select[name='field']")[0].selectedIndex=y.find("select[name='field']")[0].selectedIndex;if(G.find("select[name='op']")[0]!=null)G.find("select[name='op']").focus()[0].selectedIndex=y.find("select[name='op']")[0].selectedIndex;var x=G.find("select.vdata");if(x[0]!=null)x[0].selectedIndex=y.find("select.vdata")[0].selectedIndex}else{G.find(".data input").val("");
G.find("select[name='field']").focus()}m.datepickerFix===true&&jQuery.fn.datepicker!==undefined&&y.find(".hasDatepicker").each(function(){var N=jQuery.data(this,"datepicker").settings;G.find("#"+this.id).unbind().removeAttr("id").removeClass("hasDatepicker").datepicker(N)});G.find("select[name='field']").change();return false});b.find(".ui-search").click(function(){var y=jQuery(b.selector),G,x=y.find("select[name='groupOp'] :selected").val();G=m.stringResult?'{"groupOp":"'+x+'","rules":[':{groupOp:x,
rules:[]};y.find(".sf").each(function(N){var Y=jQuery(this).find("select[name='field'] :selected").val(),da=jQuery(this).find("select[name='op'] :selected").val(),Q=jQuery(this).find("input.vdata,select.vdata :selected").val();Q+="";Q=Q.replace(/\\/g,"\\\\").replace(/\"/g,'\\"');if(m.stringResult){if(N>0)G+=",";G+='{"field":"'+Y+'",';G+='"op":"'+da+'",';G+='"data":"'+Q+'"}'}else G.rules.push({field:Y,op:da,data:Q})});if(m.stringResult)G+="]}";m.onSearch(G);return false});b.find(".ui-reset").click(function(){var y=
jQuery(b.selector);y.find(".ui-del").click();y.find("select[name='groupOp']")[0].selectedIndex=0;m.onReset();return false});b.find(".ui-add-last").click(function(){var y=jQuery(b.selector+" .sf:last"),G=y.clone(true).insertAfter(y);G.find(".ui-state-default").removeClass("ui-state-hover ui-state-active");G.find(".data input").val("");G.find("select[name='field']").focus();m.datepickerFix===true&&jQuery.fn.datepicker!==undefined&&y.find(".hasDatepicker").each(function(){var x=jQuery.data(this,"datepicker").settings;
G.find("#"+this.id).unbind().removeAttr("id").removeClass("hasDatepicker").datepicker(x)});G.find("select[name='field']").change();return false})}}return new e(this,a,d)};jQuery.fn.searchFilter.version="1.2.9";
