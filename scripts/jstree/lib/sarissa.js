...
/*
 * * ==================================================================== About:
 * This a a compressed JS file from the Sarissa library. see
 * http://dev.abiss.gr/sarissa
 * 
 * Copyright: Manos Batsis, http://dev.abiss.gr
 * 
 * Licence: Sarissa is free software distributed under the GNU GPL version 2 or
 * higher, GNU LGPL version 2.1 or higher and Apache Software License 2.0 or
 * higher. The licenses are available online see: http://www.gnu.org
 * http://www.apache.org
 * 
...
if (Sarissa._SARISSA_IS_IE) {
	...
	if (Sarissa._SARISSA_HAS_DOM_CREATE_DOCUMENT) {
		...
		Sarissa.getDomDocument = function(sUri, sName) {
			var oDoc = document.implementation.createDocument(sUri
							? sUri
		...
		document.importNode = function(oNode, bChildren) {
			...
			if (oNode.nodeName == '#text') {
				...
				if (bChildren) {
					...
					tmp.innerHTML = oNode.xml
							? oNode.cloneNode(false).xml
...
Sarissa.xmlize = function(anyObject, objectName, indentSpace) {
	...
	if (!(anyObject instanceof Object) || anyObject instanceof Number
		...
		for (var name in anyObject) {
			s += Sarissa.xmlize(anyObject[name], (isArrayItem
							? "array-item key=\"" + name + "\""
							: name), indentSpace + "   ");
		}
		s += indentSpace;
...
Sarissa.updateContentFromNode = function(oNode, oTargetElement, xsltproc) {
	try {
		...
		var ownerDoc = oNode.nodeType == Node.DOCUMENT_NODE
				? oNode
				: oNode.ownerDocument;
