...
(function($) {
	...
	$.tree = {
		...
		reference : function(obj) {
			...
			if (!o.size())
				return null;
			o = (o.is(".tree")) ? o.attr("id") : o.parents(".tree:eq(0)")
					.attr("id");
		...
		rollback : function(data) {
			for (var i in data) {
				...
				tmp.inp = false;
				tmp.container.html(data[i].html).find(".dragged")
						.removeClass("dragged").end().find(".hover")
						.removeClass("hover");
	...
	$.fn.tree = function(opts) {
		return this.each(function() {
					...
					if (tree_component.inst
							&& tree_component.inst[$(this).attr('id')])
	...
	function tree_component() {
		return {
				...
				if (this.settings.selected !== false) {
					$.each(this.settings.selected, function(i) {
						if (_this.is_partial_refresh)
							_this.select_branch($(_this.settings.selected[i]
													.toString().replace('/',
															'\\/'),
											_this.container),
			...
			attach_events : function() {
				...
				$("#" + this.container.attr("id") + " li").live("click",
				...
				$("#" + this.container.attr("id") + " li a").live(
						"click.jstree", function(event) { // WHEN CLICK IS ON
															// THE TEXT OR ICON
							...
							event.target.blur();
							return false;
						}).live("dblclick.jstree", function(event) { // WHEN
																		// DOUBLECLICK
																		// ON
																		// TEXT
																		// OR
					...
					return _this.callback("onrgtclk", [
									_this.get_node(event.target).get(0), _this,
									event]);
					...
					if (_this.settings.rules.multiple != false
						...
						if (counter > 0) {
							...
							tree_component.drag_drop.isdown = true;
							tree_component.drag_drop.drag_help = $("<div id='jstree-dragged' class='tree "
									+ (_this.settings.ui.theme_name != ""
											? " tree-"
													+ _this.settings.ui.theme_name
											: "") + "' />")
							...
							if (_this.settings.languages.length > 0)
								tmp.find("a").not("." + _this.current_lang)
										.hide();
							tree_component.drag_drop.drag_help
									.children("ul:eq(0)").append(tmp);
							tree_component.drag_drop.drag_help
									.find("li:eq(0)")
									.removeClass("last")
									.addClass("last")
						...
						if (_this.check("draggable", obj)) {
							...
							tree_component.drag_drop.drag_help.find("li:eq(0)")
									.removeClass("last").addClass("last");
							tree_component.drag_drop.isdown = true;
			...
			select_branch : function(obj, multiple) {
				...
				if (this.settings.rules.multiple != false && !multiple) {
					for (var i in this.selected_arr) {
						if (!this.selected_arr.hasOwnProperty(i))
							continue;
						this.selected_arr[i].children("A")
								.removeClass("clicked");
				...
				this.selected.children("a").addClass("clicked").end()
			...
			open_branch : function(obj, disable_animation, callback) {
				...
				if (this.settings.data.async && obj.find("li").size() == 0) {
					...
					obj
							...
							.remove()
							.end()
							.append("<ul><li class='last'><a class='loading' href='#'><ins>&nbsp;</ins>"
			...
			create : function(obj, ref_node, position) {
					...
					for (var i = 0; i < this.settings.languages.length; i++) {
						if (!obj.data[this.settings.languages[i]])
							obj.data[this.settings.languages[i]] = ((typeof this.settings.lang.new_node)
									.toLowerCase() != "string" && this.settings.lang.new_node[i])
				...
				if ($li.children("ul").size()) {
					...
					$li.addClass("leaf");
				$li.find("li:last-child").addClass("last").end()
						.find("li:has(ul)").not(".open").addClass("closed");
				...
				var r = {
					...
					valid_children : this.check("valid_children", (root
									? -1
									: ref_node))
				};
				...
				if ((typeof position).toLowerCase() == "undefined"
						|| position == "inside")
					position = (this.settings.rules.createat == "top")
							? 0
							: ref_node.children("ul:eq(0)").children("li")
									.size();
				if (ref_node.children("ul").size() == 0
						|| (root == true && ref_node.children("ul")
								.children("li").size() == 0)) {
					if (!root)
					...
					var a = this.moved($li, ref_node.children("ul:eq(0)")
									.children("li:nth-child(" + (position)
											+ ")").children("a:eq(0)"),
							"after", true);
				else if (ref_node.children("ul:eq(0)").children("li:nth-child("
						+ (position + 1) + ")").size())
					var a = this.moved($li, ref_node.children("ul:eq(0)")
									.children("li:nth-child(" + (position + 1)
											+ ")").children("a:eq(0)"),
					...
					var a = this.moved($li, ref_node.children("ul:eq(0)")
									.children("li:last").children("a:eq(0)"),
							"after", true);
			...
			rename : function(obj, new_name) {
				...
				if ((typeof new_name).toLowerCase() == "string") {
					...
					_this.inp.val(last_value.replace(/&amp;/g, "&").replace(
							/&gt;/g, ">").replace(/&lt;/g, "<")).bind(
					...
					var spn = $("<span />").addClass(obj.attr("class"))
							.append(icn).append(_this.inp);
				...
				if (obj
					...
					if (obj.size()) {
						...
						if (obj.find("a.clicked").size()) {
							...
							_this.selected_arr = [];
							this.container.find("a.clicked")
									.filter(":first-child").parent().each(
											function() {
												if (!reset_selected
														&& this == _this.selected
																.get(0))
					...
					if (this.settings.rules.multiple == false
							|| this.selected_arr.length == 1) {
						var stop = true;
						var tmp = this.settings.ui.selected_delete == "select_previous"
			...
			moved : function(what, where, how, is_new, is_copy, rb) {
				...
				if (!rb) {
					...
					rb[this.container.attr("id")] = this.get_rollback();
					if (!is_new) {
						var tmp = what.size() > 1 ? what.eq(0)
								.parents(".tree:eq(0)") : what
								.parents(".tree:eq(0)");
				...
				if (what.size() > 1) {
					...
					what.each(function(i) {
								if (i == 0)
									return;
								if (tmp) { // if tmp is false - the previous
											// move was a no-go
									tmp = _this.moved(this, tmp
													.children("a:eq(0)"),
				...
				switch (how) {
					case "before" :
						$where.parents("ul:eq(0)").children("li.last")
								.removeClass("last");
					...
					case "inside" :
						if ($where.parent().children("ul:first").size()) {
							if (this.settings.rules.createat == "top") {
								...
								// restored this section
								var tmp_node = $where.parent()
										.children("ul:first").children(".last");
								if (tmp_node.size()) {
									how = "after";
									where = tmp_node;
								}
								...
								$where.parent().children("ul:first")
										...
										.end().append(what.removeClass("last"))
										.children("li:last").addClass("last");
							}
						} else {
							what.addClass("last");
				...
				if ($parent.find("li").size() == 0) {
					...
					$li.parents("ul:eq(0)").children("li.last")
							.removeClass("last").end().children("li:last")
							.addClass("last");
				} else {
				...
				if (is_copy)
					this
							.callback("oncopy", [this.get_node(what).get(0),
											this.get_node(where).get(0), how,
											this, rb]);
				else if (is_new)
					...
					this
							.callback("onmove", [this.get_node(what).get(0),
											this.get_node(where).get(0), how,
											this, rb]);
				return what;
			},
			...
			cut : function(obj) {
				if (this.locked)
					return this.error("LOCKED");
				obj = obj ? this.get_node(obj) : this.container
						.find("a.clicked").filter(":first-child").parent();
				if (!obj || !obj.size())
						...
						var a = this.moved(tree_component.cut_copy.copy_nodes,
								obj.children("ul:eq(0)")
										.children("li:nth-child(" + (position)
												+ ")").children("a:eq(0)"),
								"after", false, true);
					else if (obj.children("ul:eq(0)").children("li:nth-child("
							+ (position + 1) + ")").size())
						var a = this.moved(tree_component.cut_copy.copy_nodes,
								obj.children("ul:eq(0)")
										.children("li:nth-child("
			...
			destroy : function() {
				...
				$("#" + this.container.attr("id")).die("click.jstree")
						.die("dblclick.jstree").die("mouseover.jstree")
						.die("mouseout.jstree").die("mousedown.jstree");
				this.container
						.removeClass("tree ui-widget ui-widget-content tree-default tree-"
								+ this.settings.ui.theme_name).children("ul")
						.removeClass("no_dots ltr locked").find("li")
						.removeClass("leaf").removeClass("open")
						.removeClass("closed").removeClass("last")
						.children("a").removeClass("clicked hover search");
	...
	tree_component.mouseup = function(event) {
		...
		if (tmp.foreign === false && tmp.drag_node && tmp.drag_node.size()) {
			...
			if (tmp.move_type) {
				var tree1 = tree_component.inst[tmp.ref_node
						.parents(".tree:eq(0)").attr("id")];
				if (tree1)
					tree1
							.moved(
									tmp.dragged,
		...
		"html" : function() {
			return {
				get : function(obj, tree, opts) {
					return obj && $(obj).size() ? $('<div>').append(tree
							.get_node(obj).clone()).html() : tree.container
							.children("ul:eq(0)").html();
				},
				...
				load : function(data, tree, opts, callback) {
					if (opts.url) {
						$.ajax({
									...
									'error' : function(xhttp, textStatus,
											errorThrown) {
						...
						callback.call(null, opts.static
										|| tree.container.children("ul:eq(0)")
												.html());
					}
				}
			};
				...
				get : function(obj, tree, opts) {
					...
					for (var i in opts.outer_attrib) {
						if (!opts.outer_attrib.hasOwnProperty(i))
							continue;
						var val = (opts.outer_attrib[i] == "class")
								? obj.attr(opts.outer_attrib[i]).replace(
										/(^| )last( |$)/ig, " ").replace(
										/(^| )(leaf|closed|open)( |$)/ig, " ")
					...
					if (tree.settings.languages.length) {
							...
							if (opts.force
								...
								if (a.children("ins").get(0).style.className.length) {
									json.data[tree.settings.languages[i]].icon = a
											.children("ins").get(0).style.className;
								}
								if (a.children("ins").get(0).style.backgroundImage.length) {
									json.data[tree.settings.languages[i]].icon = a
											.children("ins").get(0).style.backgroundImage
											.replace("url(", "").replace(")",
													"");
								}
						...
						if (a.children("ins").size()
							...
							json.data.icon = a.children("ins").get(0).style.backgroundImage
									.replace("url(", "").replace(")", "");
						}
					...
					if (obj.children("ul").size() > 0) {
						...
						obj.children("ul").children("li").each(function() {
									json.children.push(_this.get(this, tree,
											opts));
								});
					}
					return json;
				...
				parse : function(data, tree, opts, callback) {
					...
					if (tree.settings.languages.length) {
						...
						if (data.data.icon) {
							...
							str += "<ins>&nbsp;</ins>";
						str += ((typeof data.data.title).toLowerCase() != "undefined"
								? data.data.title
								: data.data)
								+ "</a>";
					}
