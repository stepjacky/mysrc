		...
		"contextmenu" : {
			...
			show : function(obj, t) {
				...
				for (var i in opts.items) {
					...
					if (typeof opts.items[i].visible == "function")
						r = opts.items[i].visible.call(null,
								$.tree.plugins.contextmenu.data.a, t);
					if (r == -1)
						continue;
					else
						cnt++;
					...
					str += '<li><a href="#" rel="' + i + '" class="' + i + ' '
							+ (r == 0 ? 'disabled' : '') + '">';
					if (opts.items[i].icon)
						str += "<ins "
								+ (opts.items[i].icon.indexOf("/") == -1
										? " class='" + opts.items[i].icon
												+ "' "
			...
			hide : function() {
				...
				if ($.tree.plugins.contextmenu.data.r
					...
					$.tree.plugins.contextmenu.data.a.children("a, span")
							.removeClass(opts.class_name);
				}
			...
			exec : function(cmd) {
				...
				var opts = $
						.extend(
								...
								$.tree.plugins.contextmenu.data.t.settings.plugins.contextmenu);
				try {
					opts.items[cmd].action.apply(null, [
									$.tree.plugins.contextmenu.data.a,
			...
			callbacks : {
				...
				onrgtclk : function(n, t, e) {
					...
					if (!n.children("a:eq(0)").hasClass("clicked")) {
						...
						$.tree.plugins.contextmenu.data.a = (t.selected_arr && t.selected_arr.length > 1)
								? t.selected_arr
								: t.selected;
					}
					...
					e.preventDefault();
					e.stopPropagation();
					// return false; // commented out because you might want to
					// do something in your own callback
				},
				onchange : function() {
