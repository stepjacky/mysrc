		...
		"xml_nested" : function() {
			return {
					...
					if (tree.settings.languages.length) {
						for (var i in tree.settings.languages) {
							if (!tree.settings.languages.hasOwnProperty(i))
								continue;
							str += this.process_inner(obj.children("a."
											+ tree.settings.languages[i]),
									tree, opts, tree.settings.languages[i]);
						}
					} else {
						str += this
								.process_inner(obj.children("a"), tree, opts);
					}
					str += '</content>';
				...
				process_inner : function(obj, tree, opts, lang) {
					...
					if (opts.inner_attrib.length
						...
						if (obj.children("ins").get(0).style.className.length) {
							str += ' icon="'
									+ obj.children("ins").get(0).style.className
									+ '" ';
						}
						...
						if (opts.inner_attrib.length) {
							for (var j in opts.inner_attrib) {
								...
								if (typeof val != "undefined"
										&& val.toString().replace(" ", "").length > 0)
									str += ' ' + opts.inner_attrib[j] + '="'
											+ val.toString() + '" ';
								delete val;
							}
						}
					}
					str += '><![CDATA[' + tree.get_text(obj, lang)
							+ ']]></name>';
					return str;
				},
				parse : function(data, tree, opts, callback) {
					...
					var result = $((new XMLSerializer())
							.serializeToString(processor
									.transformToDocument(data)).replace(
									'<?xml version="1.0"?>', ''));
					// Opera bug
					if (result.size() > 1)
						result = result.eq(1);
				...
				load : function(data, tree, opts, callback) {
					if (opts.static) {
						callback.call(null, (new DOMParser()).parseFromString(
										opts.static, 'text/xml'));
					} else {
						$.ajax({
									'type' : opts.method,
									'url' : opts.url,
