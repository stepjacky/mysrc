...
function doUpdate(){
    ...
    $entryName = $util->getEntryName(basename(__FILE__));
    $dao  =new BaseDao($entryName);
    $vals = array("name"=>$_POST['name'],'intro'=>$_POST['intro'],"isStreet"=>$_POST['isStreet']);
...
function doList(){
    ...
    if(isset($_POST['actionparam']) && $_POST['actionparam']=='all'){
        $SQL = "SELECT id,name,isStreet,intro FROM $entryName";
    ...
    while($row = mysql_fetch_assoc($result)) {
        ...
        $responce->rows[$i]['isstreet']=$row['isstreet'];
        $i++;
    }
