...
function doDelete(){
    ...
    $SQL = array();
    $id = $_POST['id'];
    $ts  = "delete from commoncommit where artitle_id=$id";
    array_push($SQL, $ts);
    $ts = "delete from artitle where id=$id";
    array_push($SQL,$ts);
...
function doList(){
    ...
    $SQL = "SELECT a.id id ,a.title title ,a.publishdate publishdate, ac.name cname 
            FROM $entryName a inner join artitleCatalog ac on ac.id=a.artitleCatalog_id 
